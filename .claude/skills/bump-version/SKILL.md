---
name: bump-version
description: Use when bumping the CoraCowork version: query CoraCore release, verify artifacts, update package.json, generate CHANGELOG, branch, commit, push, create PR, auto-merge, tag release.
---

# Bump Version

Automate the CoraCowork release preparation: query CoraCore release → verify artifacts → update versions → generate CHANGELOG → branch → PR → tag.

**Usage:** `/bump-version [version] [flags]`

- `/bump-version` — auto patch + latest CoraCore
- `/bump-version 2.2.0` — explicit CoraCowork version + latest CoraCore
- `/bump-version 2.2.0 --core v0.1.12` — explicit both versions
- `/bump-version --skip-core` — pure frontend release (don't touch coraCoworkVersion)
...
### Step 3: Determine CoraCowork Target Version
...
Display: "Bumping CoraCowork: {current} → {target}"

### Step 4: Query CoraCore Latest Release
...
- Display the CoraCore version and ask user to confirm before continuing
- Also read current `coraCoworkVersion` from `package.json` — if it already matches the queried version, warn the user and ask whether to proceed or use `--skip-core`

### Step 5: Verify CoraCore Artifacts
...
- `CoraCore-<tag>-x86_64-unknown-linux-gnu.tar.gz`
- `CoraCore-<tag>-aarch64-unknown-linux-gnu.tar.gz`
- `CoraCore-<tag>-x86_64-apple-darwin.tar.gz`
- `CoraCore-<tag>-aarch64-apple-darwin.tar.gz`
- `CoraCore-<tag>-x86_64-pc-windows-msvc.zip`
- `CoraCore-<tag>-aarch64-pc-windows-msvc.zip`
- `cora-cowork-checksums.txt`

Missing → Stop: "CoraCore {tag} is missing artifacts: {list}. Wait for CI to complete or check for build failures."
...
- `"coraCoworkVersion": "{old}"` → `"coraCoworkVersion": "{new core tag}"` (skip if `--skip-core`)
...
## [{target}](https://github.com/coracowork/cora-cowork-desktop/compare/v{previous}...v{target}) ({date YYYY-MM-DD})
...
git commit -m "chore: bump version to {target} and CoraCore to {core tag}"
...
```
 ...
 2. git pull --rebase
 3. Determine CoraCowork target version (patch+1 or explicit)
 4. Query CoraCore latest release (or --core / --skip-core)
 5. Verify CoraCore artifacts (7 files)
 6. Edit package.json (version + coraCoworkVersion)
 7. Generate CHANGELOG entry (frontend commits + CoraCore release body)
 8. lint + format + tsc
 9. vitest run
