...
- **Rust stable + Cargo** — Required to build the local CoraCore backend ([install](https://rustup.rs))
...
The desktop development server resolves the backend from the `PATH` inherited by `bun run start`. Install CoraCore first, verify the binary is discoverable in the same terminal, then start CoraCowork.
...
```bash
git clone https://github.com/iOfficeAI/CoraCore.git
git clone https://github.com/coracowork/cora-cowork-desktop.git
```

Use the `main` branch for both repositories unless a maintainer asks you to test another branch.

### 2. Build and Install CoraCore

Run these commands from the `CoraCore` repository.
...
cd CoraCore
cargo clean
cargo install --path crates/cora-cowork-app --locked

# Make Cargo-installed binaries visible to this shell if needed.
export PATH="$HOME/.cargo/bin:$PATH"

# Verify that CoraCowork will be able to find the backend.
which CoraCore
CoraCore --help
```

If `which CoraCore` prints nothing, add `export PATH="$HOME/.cargo/bin:$PATH"` to your shell profile (`~/.zshrc`, `~/.bashrc`, or your shell's equivalent), open a new terminal, and verify again.
...
If `where.exe CoraCore` prints nothing, make sure `%USERPROFILE%\.cargo\bin` is in your user `Path`, open a new PowerShell window, and verify again.

### 3. Start CoraCowork

Run these commands from the `CoraCowork` repository in a terminal where `CoraCore` is discoverable.
...
During startup, CoraCowork launches `CoraCore` automatically and passes the backend port to the renderer. You do not need to start CoraCore in a separate terminal.

## Updating the Local Backend

When you pull or change CoraCore, reinstall the backend binary and restart CoraCowork:

```bash
cd ../CoraCore
cargo install --path crates/cora-cowork-app --locked --force

cd ../CoraCowork
bun run start
```

Use `--force` when rebuilding local changes with the same CoraCore package version; otherwise Cargo may keep the already installed binary.

## Backend Startup Troubleshooting

### `Cannot find "CoraCore" binary`

CoraCowork cannot find the backend from the `PATH` inherited by `bun run start`.

Check from the same terminal where you start CoraCowork:
...
If the command fails, add Cargo's binary directory to `PATH` and start CoraCowork from a new terminal.

### `CoraCore` Works in a Terminal but CoraCowork Still Cannot Find It

Make sure you start `bun run start` from the same terminal environment that can run `CoraCore --help`. IDE terminals and GUI-launched shells can inherit a different `PATH`; restart the IDE or launch it from a terminal after updating `PATH`.

### Backend Changes Do Not Show Up

Quit CoraCowork, reinstall CoraCore with `cargo install --path crates/cora-cowork-app --locked --force`, then start CoraCowork again. The Electron app owns the backend subprocess during development, so a running CoraCowork instance will not pick up a newly installed binary until it restarts.

### Windows Rust Build Errors

Use the Rust MSVC toolchain and install Microsoft C++ Build Tools. After installing or changing toolchains, open a new PowerShell window and rerun the CoraCore install command.
...
When you have two clones of the repository (e.g. `CoraCowork` and `CoraCowork-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `CORA_COWORK_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`CoraCowork-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.cora-cowork-dev-2`, `~/.cora-cowork-config-dev-2`)
...
CoraCowork uses **electron-vite** for fast bundling:
