...
$log = $LogPath
if (-not $log) {
  $log = Join-Path $env:TEMP 'cora-cowork-installer-fallback-log.jsonl'
}
$logFileName = Split-Path -Leaf $log
$statusPath = Join-Path $env:TEMP 'cora-cowork-installer-report.json'
...
function Read-CoraCoworkAnalyticsId {
  $candidates = @()
  if ($env:APPDATA) {
    $candidates += (Join-Path $env:APPDATA 'CoraCowork\analytics.json')
  }
  if ($env:LOCALAPPDATA) {
    $candidates += (Join-Path $env:LOCALAPPDATA 'CoraCowork\analytics.json')
  }
...
function Get-OptionalHandleDiagnostics {
  ...
  $outPath = Join-Path $env:TEMP ('cora-cowork-handle-' + [guid]::NewGuid().ToString('N') + '.out')
  $errPath = Join-Path $env:TEMP ('cora-cowork-handle-' + [guid]::NewGuid().ToString('N') + '.err')
...
function New-ReportDetailsText(
  ...
  $lines.Add('--------------------------------')
  $lines.Add('CoraCowork installer failure ' + $code)
  $lines.Add('--------------------------------')
  $lines.Add('')
  if ($eventId) {
...
try {
  ...
  $userId = Read-CoraCoworkAnalyticsId
  $eventPayload = @{
    message = ('installer-failure ' + $Code)
    level = 'error'
    platform = 'other'
    release = $Release
    logger = 'cora-cowork.installer'
  ...
  Show-ReportMessage ('CoraCowork installer report sent.' + [Environment]::NewLine + [Environment]::NewLine + 'Tip: If you can get in touch with the CoraCowork team, press [ Ctrl + C ] in this dialog to copy details, then send them to us via social media, email, or a GitHub issue:' + [Environment]::NewLine + 'https://github.com/coracowork/cora-cowork-desktop/issues' + [Environment]::NewLine + [Environment]::NewLine + $reportDetails) 'Information'
} catch {
  ...
  Show-ReportMessage ('CoraCowork installer report failed.' + [Environment]::NewLine + [Environment]::NewLine + 'Status: ' + $statusPath + [Environment]::NewLine + 'Log: ' + $log) 'Exclamation'
}
