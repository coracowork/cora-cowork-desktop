import { startWebHost, startStaticServer } from '@cora-cowork/web-host';
import type { WebHostHandle, StaticServerHandle } from '@cora-cowork/web-host';
...
function resolveCliRoot(): string {
  // Heuristic: if the executable path ends in "cora-cowork-web" or "cora-cowork-web.exe",
  // treat it as the packaged single-file binary and return its directory.
  const exe = process.execPath;
  const exeName = path.basename(exe).toLowerCase();
  if (exeName === 'cora-cowork-web' || exeName === 'cora-cowork-web.exe') {
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.CORA_COWORK_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.cora-cowork-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.CORA_COWORK_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolvePort(flags: Map<string, string | true>): number {
  ...
  const env = process.env.CORA_COWORK_PORT ?? process.env.PORT;
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.CORA_COWORK_ALLOW_REMOTE ?? process.env.CORA_COWORK_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[cora-cowork-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[cora-cowork-web] version    : ${version}`);
  console.log(`[cora-cowork-web] data dir   : ${dataDir}`);
  console.log(`[cora-cowork-web] log dir    : ${logDir}`);
  console.log(`[cora-cowork-web] static dir : ${staticDir}`);
  console.log(`[cora-cowork-web] backend bin: ${backendBin}`);
  console.log(`[cora-cowork-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download CoraCore and set CORA_COWORK_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('CoraCowork WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[cora-cowork-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[cora-cowork-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
    ...
    console.log('');
    console.log('CoraCowork WebUI is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[cora-cowork-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[cora-cowork-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `cora-cowork-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[cora-cowork-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set CORA_COWORK_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[cora-cowork-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[cora-cowork-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[cora-cowork-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[cora-cowork-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[cora-cowork-web] username: ${username}`);
    console.log(`[cora-cowork-web] new password: ${newPassword}`);
    console.log('[cora-cowork-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: cora-cowork-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.cora-cowork-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.cora-cowork-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  CORA_COWORK_PORT, CORA_COWORK_ALLOW_REMOTE, CORA_COWORK_DATA_DIR, CORA_COWORK_LOG_DIR,
  CORA_COWORK_BACKEND_BIN, CORA_COWORK_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: cora-cowork-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[cora-cowork-web] fatal:', err.message);
