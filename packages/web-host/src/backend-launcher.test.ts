...
function emitListening(child: ChildProcess, port: number): void {
  child.stdout?.emit('data', Buffer.from(`CoraCore_LISTENING {"host":"127.0.0.1","port":${port}}\n`));
}
...
describe('buildSpawnArgs', () => {
  ...
  it('passes prompt dump flag in development only when CORA_COWORK_DUMP_PROMPTS is enabled', () => {
    const prev = process.env.CORA_COWORK_DUMP_PROMPTS;
    process.env.CORA_COWORK_DUMP_PROMPTS = '1';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.CORA_COWORK_DUMP_PROMPTS;
      else process.env.CORA_COWORK_DUMP_PROMPTS = prev;
    }
  });
  ...
  it('respects CORA_COWORK_LOG_LEVEL override', () => {
    const prev = process.env.CORA_COWORK_LOG_LEVEL;
    process.env.CORA_COWORK_LOG_LEVEL = 'trace';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.CORA_COWORK_LOG_LEVEL;
      else process.env.CORA_COWORK_LOG_LEVEL = prev;
    }
  });
});

describe('buildSpawnEnv', () => {
  it('merges process.env with CORA_COWORK_* dir vars', () => {
...
describe('findAvailablePort', () => {
  ...
  it('skips ports blocked by Fetch so health checks can use the selected port', async () => {
    ...
    try {
      ...
      expect(infoSpy).toHaveBeenCalledWith('[coracore] skipped fetch-blocked backend port 1720');
      expect(infoSpy).toHaveBeenCalledWith('[coracore] selected backend port 40404 after 2 attempts');
    } finally {
      infoSpy.mockRestore();
    }
  });
...
describe('BackendLifecycleManager.start (success path)', () => {
  it('lets coracore choose the backend port and waits for the reported listening event', async () => {
    vi.mocked(createServer).mockImplementation(() => {
    ...
    const mgr = new BackendLifecycleManager(APP_META_PACKAGED, () => '/abs/path/coracore');
    ...
    child.stdout?.emit('data', Buffer.from('CoraCore_LISTENING {"host":"127.0.0.1","port":55555}\n'));
  ...
  it('spawns with correct args, waits for /health, reports running', async () => {
    ...
    const resolveBackend = vi.fn(() => '/abs/path/coracore');
    ...
    try {
      ...
      expect(spawnCall[0]).toBe('/abs/path/coracore');
      ...
      expect(infoSpy).toHaveBeenCalledWith(
        expect.stringContaining('[coracore] health ready on port 55555 after 1 attempts, elapsed_ms=')
      );
    } finally {
      fetchSpy.mockRestore();
      infoSpy.mockRestore();
...
describe('BackendLifecycleManager.start (health timeout)', () => {
  ...
  it('captures backend boundary code and stage from early-exit stderr', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.open databasePath=/db/path/cora-cowork-backend.db: failed to initialize application data\n'
      )
    );
  ...
  it('captures backend boundary code when stderr drains after exit but before close', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.migration databasePath=/db/path/cora-cowork-backend.db: failed to initialize application data\n'
      )
    );
    child.emit('close', 1, null);
  ...
  it('kills child and reports listen_timeout when coracore never reports a port', async () => {
    vi.useFakeTimers();
  ...
  it('records when server listening appears before health check times out', async () => {
    ...
    const expectedRejection = expect(startPromise).rejects.toMatchObject({
      ...
      details: expect.objectContaining({
        ...
        serverListeningLine: expect.stringContaining('CoraCore_LISTENING'),
      }),
    });
...
describe('BackendLifecycleManager crash restart', () => {
  ...
  it('logs crash restart scheduling details', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[coracore] child exited unexpectedly; scheduling restart', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 65303,
      restartCount: 1,
  ...
  it('logs when crash restart limit is exceeded', () => {
    ...
    expect(mgr.status).toBe('error');
    expect(errorSpy).toHaveBeenCalledWith('[coracore] child exited unexpectedly; restart limit exceeded', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 0,
      restartCount: 4,
