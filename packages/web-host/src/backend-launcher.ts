/**
 * Lifecycle manager for the coracore subprocess (web-host version).
 *
...
export type BackendLaunchOptions = {
  ...
  logDir?: string;
  /**
   * System dirs exposed to the backend via CORA_COWORK_{CACHE,WORK,LOG}_DIR env.
...
export class BackendStartupCancelledError extends Error {
  constructor(message = 'coracore startup cancelled') {
    super(message);
...
export function buildSpawnArgs(config: SpawnConfig): string[] {
  const logLevel = process.env.CORA_COWORK_LOG_LEVEL || (config.isPackaged ? 'info' : 'debug');
  const args = [
    '--port',
    String(config.port),
    '--data-dir',
  ...
  if (!config.isPackaged && process.env.CORA_COWORK_DUMP_PROMPTS === '1') args.push('--dump-prompts');
...
/**
 * Backend reads CORA_COWORK_{CACHE,WORK,LOG}_DIR env vars to report system dirs
 * (see CoraCore/crates/coracowork-system/src/sysinfo.rs). Inject them so the
 * backend's `/api/system/info` matches what Electron main persists in
 * ProcessEnv('coracowork.dir').
 */
export function buildSpawnEnv(dirs: BackendDirConfig): NodeJS.ProcessEnv {
  return {
    ...process.env,
    _CACHE_DIR: dirs.cacheDir,
    CORA_COWORK_WORK_DIR: dirs.workDir,
    CORA_COWORK_LOG_DIR: dirs.logDir,
  };
}
...
const CoraCore_LISTENING_PREFIX = 'CORACORE_LISTENING ';
const CoraCore_LISTENING_PREFIX_LEGACY = 'CoraCore_LISTENING ';
...
export function findAvailablePort(
  ...
  if (preferredPort && firstRequestedPort === 0) {
    console.info(`[coracore] skipped fetch-blocked backend port ${preferredPort}`);
  }
      ...
      server.listen(requestedPort, '127.0.0.1', () => {
        ...
        server.close(() => {
          ...
          if (resolvedPort > 0 && !isFetchForbiddenPort(resolvedPort)) {
            console.info(`[coracore] selected backend port ${resolvedPort} after ${attempt} attempts`);
            resolve(resolvedPort);
            return;
          }
          if (resolvedPort > 0 && remainingAttempts > 1) {
            console.info(`[coracore] skipped fetch-blocked backend port ${resolvedPort}`);
...
function parseCoracoreListeningPort(line: string): number | undefined {
  // coracore may print the listening marker either as a standalone line or
  // embedded in a structured tracing log (e.g. "... startup: CORACORE_LISTENING {...}").
  // The casing also varies across versions, so search for all variants.
  const lowerLine = line.toLowerCase();
  for (const prefix of [CoraCore_LISTENING_PREFIX, CoraCore_LISTENING_PREFIX_LEGACY]) {
    const idx = lowerLine.indexOf(prefix.toLowerCase());
    if (idx === -1) continue;
    const rest = line.slice(idx + prefix.length);
    try {
      const parsed = JSON.parse(rest) as { port?: unknown; host?: unknown };
      ...
      // rest may contain trailing text after the JSON object; try to extract it.
      ...
      try {
        const parsed = JSON.parse(rest.slice(jsonStart, jsonEnd + 1)) as { port?: unknown };
        if (typeof parsed.port !== 'number' || !Number.isInteger(parsed.port)) return undefined;
        if (parsed.port <= 0 || parsed.port > 65535) return undefined;
        return parsed.port;
      } catch {
...
function ensureBackendStartupDirectory(dir: string | undefined): void {
  ...
  try {
    ...
    // Directory may already exist and be accessible (e.g. drive root on Windows)
    // or may fail for transient reasons — neither should block backend startup.
  }
}
...
export class BackendLifecycleManager {
  ...
  async start(
    ...
    try {
      ...
      throw new BackendStartupError(
        'coracore startup failed while resolving backend binary',
        {
    ...
    console.log(`[coracore] starting: ${binaryPath} ${args.join(' ')}`);
    ...
    try {
      ...
      this._status = 'error';
      throw makeStartupError('spawn', 'coracore startup directory preparation failed', error);
    }
    ...
    try {
      ...
      this._status = 'error';
      throw makeStartupError('spawn', 'coracore process spawn threw before startup', error);
    }
    ...
    const startupFailure = new Promise<never>((_resolve, reject) => {
      ...
      this.childProcess?.once('error', (error) => {
        ...
        this._status = 'error';
        rejectOnce(makeStartupError('spawn_error', 'coracore process emitted an error before startup', error));
      });
      ...
      this.childProcess?.once('close', (code, signal) => {
        ...
        if (!pendingStartupExit.startupSettledAtExit) {
          if (pendingStartupExit.statusAtExit === 'stopped') {
            rejectOnce(new BackendStartupCancelledError('coracore startup cancelled before health check passed'));
            return;
          }
          rejectOnce(
            makeStartupError('early_exit', 'coracore exited before health check passed', undefined, {
        ...
        if (pendingStartupExit.statusAtExit === 'starting') {
          void Promise.resolve(
            options?.onPendingExit?.(
              makeStartupError('early_exit', 'coracore exited after startup health timeout', undefined, {
                exitCode: exitCode ?? undefined,
                signal: exitSignal ?? undefined,
              })
            )
          ).catch((error) => {
            console.error('[coracore] pending exit handler failed:', error);
          });
        }
      });
    });
    ...
    const reportedPort = new Promise<number>((resolve, reject) => {
      ...
      reportedPortTimer = setTimeout(() => {
        rejectReportedPort(
          makeStartupError('listen_timeout', 'coracore did not report its listening port before timeout', undefined, {
    ...
    this.childProcess.stdout?.on('data', (data: Buffer) => {
      ...
      for (const line of data.toString().split('\n')) {
        ...
        const port = parseCoracoreListeningPort(trimmed);
        if (port !== undefined) {
          this._port = port;
        ...
        if (trimmed) console.log(`[coracore] ${line}`);
      }
    });
    ...
    this.childProcess.stderr?.on('data', (data: Buffer) => {
      ...
      for (const line of data.toString().split('\n')) {
        if (line.trim()) console.error(`[coracore] ${line}`);
      }
    });
    ...
    if (!health.ok) {
      const healthTimeoutError = makeStartupError(
        'health_timeout',
        'coracore failed to start within timeout',
        undefined,
        {
          ...health.diagnostics,
        }
      ...
      if (options?.allowPendingOnHealthTimeout && this.childProcess) {
        startupSettled = true;
        console.warn(`[coracore] health check timed out; keeping process alive on port ${this._port}`);
        void Promise.resolve(options.onHealthTimeout?.(healthTimeoutError)).catch((error) => {
          console.error('[coracore] health timeout handler failed:', error);
        });
    ...
    this.restartCount = 0;
    console.info(
      `[coracore] health ready on port ${this._port} after ${health.diagnostics.healthCheckAttempts} attempts, elapsed_ms=${health.diagnostics.healthCheckElapsedMs}, data-dir: ${dbPath}`
    );
    return this._port;
  }
  ...
  private continueWaitingForHealth(
    ...
    void (async () => {
      ...
      console.info(
        `[coracore] late health ready on port ${port} after ${health.diagnostics.healthCheckAttempts} attempts, elapsed_ms=${elapsedMs}, data-dir: ${this._lastDbPath}`
      );
      await onReady?.(port);
    })().catch((error) => {
      console.error('[coracore] background health wait failed:', error);
    });
  }
  ...
  private handleCrash(code: number | null, signal?: NodeJS.Signals | string | null): void {
    ...
    if (this.restartCount > this.maxRestarts) {
      this._status = 'error';
      console.error('[coracore] child exited unexpectedly; restart limit exceeded', crashContext);
      return;
    }

    const delay = Math.pow(2, this.restartCount - 1) * 1000;
    console.warn('[coracore] child exited unexpectedly; scheduling restart', {
      ...crashContext,
      delayMs: delay,
    });
    ...
    setTimeout(() => {
      ...
      this.start(this._lastDbPath, this._lastLogDir, this._lastDirs, this._lastOptions, this._port)
        ...
        .catch((error) => {
          this._status = 'error';
          console.error('[coracore] restart after crash failed', {
            port: this._port,
            restartCount: this.restartCount,
            maxRestarts: this.maxRestarts,
            delayMs: delay,
