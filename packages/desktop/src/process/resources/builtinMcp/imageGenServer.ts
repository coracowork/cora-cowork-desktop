...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env.CORA_COWORK_IMG_PLATFORM;
  const base_url = process.env.CORA_COWORK_IMG_BASE_URL;
  const api_key = process.env.CORA_COWORK_IMG_API_KEY;
  const model = process.env.CORA_COWORK_IMG_MODEL;
...
IMPORTANT: When user provides multiple images, ALWAYS pass ALL images to the image_uris parameter as an array.`,
    ...
    async ({ prompt, image_uris, workspace_dir }) => {
      ...
      const proxy = process.env.CORA_COWORK_IMG_PROXY || undefined;
