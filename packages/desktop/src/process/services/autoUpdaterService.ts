...
import { buildGitHubFeedOptions } from './updateFeed';

const FORCE_DEV_AUTO_UPDATE_ENV = 'CORA_COWORK_FORCE_DEV_AUTO_UPDATE';
...
class AutoUpdaterService extends EventEmitter {
  ...
  constructor() {
    ...
    log.info('Update feed set to GitHub provider');
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        ...
        `owner: ${githubFeedOptions.owner}`,
        `repo: ${githubFeedOptions.repo}`,
        'updaterCacheDirName: com.cora-cowork.app',
        '',
      ].join('\n');
  ...
  private moveCwdOutOfInstallDirForWindowsHandoff(): void {
    ...
    try {
      const safeCwd = path.join(app.getPath('temp'), 'cora-cowork-updater-cwd');
   ...
   * while the downloaded archive contains the packaged app (com.cora-cowork.app). Squirrel.Mac
