...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'cora-cowork');
};
 ...
 * Release builds use ~/.cora-cowork; dev builds use ~/.cora-cowork-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.cora-cowork，Dev 模式使用 ~/.cora-cowork-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'cora-cowork');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.cora-cowork'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.cora-cowork-config; dev builds use ~/.cora-cowork-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.cora-cowork-config，Dev 模式使用 ~/.cora-cowork-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.cora-cowork-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[CoraCowork] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[CoraCowork] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[CoraCowork] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[CoraCowork] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
