...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.CORA_COWORK_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.CORA_COWORK_ALLOW_REMOTE || process.env.CORA_COWORK_REMOTE);
  const hostHint = process.env.CORA_COWORK_HOST?.trim();
...
// 🔥 MODIFICADO: Retorna URL do renderer usando a porta real do servidor
const toDesktopHandle = (handle: WebHostHandle, allowRemote: boolean): DesktopWebUIHandle => {
  ...
  return {
    ...
    networkUrl: lanIP ? `http://${lanIP}:${port}` : undefined,
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  // Resolve allowRemote: explicit opt > env var > config (readWebUIDesktopPreferences already ran)
  const envRemote = parseBooleanEnv(process.env.CORA_COWORK_ALLOW_REMOTE || process.env.CORA_COWORK_REMOTE);
  const allowRemote = opts.allowRemote === true || envRemote === true;
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns ~/.cora-cowork[-dev] symlink on macOS to sidestep
  ...
  // If remote access is enabled, try to open the port on the router via UPnP.
  // This is best-effort — not all routers support it.
  if (allowRemote) {
    addPortMapping(handle.port).catch(() => {});
  }
...
export async function stopDesktopWebUI(): Promise<void> {
  ...
  // Remove UPnP port mapping if remote access was enabled
  if (handle.allowRemote) {
    removePortMapping(handle.port).catch(() => {});
  }
...
export function getDesktopWebUIStatus(): {
  ...
  if (!currentHandle) {
    return {
      ...
      networkUrl: lanIP ? `http://${lanIP}:${DEFAULT_WEBUI_PORT}` : undefined,
      lanIP: lanIP ?? undefined,
    };
  }
  const port = currentHandle.port;
  return {
    running: true,
    port,
    ...
    localUrl: `http://localhost:${port}`,
    networkUrl: lanIP ? `http://${lanIP}:${port}` : undefined,
    lanIP: currentHandle.lanIP || lanIP || undefined,
    initialPassword: currentInitialPassword,
  };
}
