...
const STORAGE_PATH = {
  config: 'cora-cowork-config.txt',
  chatMessage: 'cora-cowork-chat-message.txt',
  chat: 'cora-cowork-chat.txt',
  env: '.cora-cowork-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[CoraCowork] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[CoraCowork] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[CoraCowork] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('cora-cowork.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'cora-cowork-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'cora-cowork-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'cora-cowork-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'cora-cowork-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[CoraCowork] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `crates/cora-cowork-app/assets/builtin-assistants/` and
 * `crates/cora-cowork-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[CoraCowork:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog (see cora-cowork-assistant crate) and
  ...
  try {
    ...
    console.error('[CoraCowork] Failed to ensure assistant dirs:', error);
  }
