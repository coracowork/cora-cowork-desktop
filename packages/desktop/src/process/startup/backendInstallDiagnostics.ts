...
export function collectBackendInstallDiagnostics(
  ...
  const checkedBundledPath = getString(details?.checkedBundledPath);
  const runtimeDirPathFromChecked = checkedBundledPath ? pathApi.dirname(checkedBundledPath) : undefined;
  const bundledDirPathFromChecked = runtimeDirPathFromChecked ? pathApi.dirname(runtimeDirPathFromChecked) : undefined;
  const bundledDirPath =
    bundledDirPathFromChecked ?? (resourcesPath ? pathApi.join(resourcesPath, BUNDLED_CoraCore_DIR) : undefined);
  const runtimeDirPath =
    runtimeDirPathFromChecked ??
    (resourcesPath && runtimeKey ? pathApi.join(resourcesPath, BUNDLED_CoraCore_DIR, runtimeKey) : undefined);
  const binaryPath =
    checkedBundledPath ??
