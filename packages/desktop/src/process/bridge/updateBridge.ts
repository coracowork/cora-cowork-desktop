...
const DEFAULT_REPO = 'coracowork/cora-cowork-desktop';
const DEFAULT_USER_AGENT = 'coracowork';
const ALLOWED_ASSET_EXTS = new Set(['.exe', '.msi', '.dmg', '.zip', '.deb', '.rpm']);
const CDN_HOST = 'github.com';
const CDN_BASE_URL = `https://api.github.com/repos/${DEFAULT_REPO}/releases`;
...
/**
 * Use the GitHub release asset browser_download_url directly.
 */
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = process.env.CORA_COWORK_GITHUB_REPO?.trim();
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `CoraCowork-update-${Date.now()}`;
};
