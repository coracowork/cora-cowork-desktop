...
function isBackendStartupFailureEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.['cora-cowork.failure'] === 'backend_startup';
}

function isUserFeedbackEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.type === 'user-feedback' || event.tags?.['cora-cowork.installation_integrity.user_report'] === 'true';
}
...
function getInstallPathKind(resourcesPath: unknown): string | undefined {
  ...
  if (normalized.includes('\\appdata\\local\\programs\\cora-cowork\\resources')) {
    return 'user_local_programs';
  }
  if (
    normalized.includes('\\program files\\cora-cowork\\resources') ||
    normalized.includes('\\program files (x86)\\cora-cowork\\resources')
  ) {
...
export async function captureBackendStartupFailure(error: unknown): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.setTag('cora-cowork.failure', 'backend_startup');
    scope.setTag('cora-cowork.backend_startup.reason', failureInfo.reason);
    if (failureInfo.runtime) {
      scope.setTag('cora-cowork.backend_startup.runtime', failureInfo.runtime);
    }
    if (failureInfo.packageArch) {
      scope.setTag('cora-cowork.backend_startup.package_arch', failureInfo.packageArch);
    }
    if (failureInfo.deviceArch) {
      scope.setTag('cora-cowork.backend_startup.device_arch', failureInfo.deviceArch);
    }
    if (failureInfo.expectedDownloadArch) {
      scope.setTag('cora-cowork.backend_startup.expected_download_arch', failureInfo.expectedDownloadArch);
    }
    if (typeof failureInfo.isRosettaTranslated === 'boolean') {
      scope.setTag('cora-cowork.backend_startup.rosetta_translated', getBooleanTagValue(failureInfo.isRosettaTranslated));
    }
    if (typeof details?.stage === 'string') {
      scope.setTag('cora-cowork.backend_startup.stage', details.stage);
    }
    if (failureInfo.backendBoundaryCode) {
      scope.setTag('cora-cowork.backend_startup.boundary_code', failureInfo.backendBoundaryCode);
    }
    if (failureInfo.backendBoundaryStage) {
      scope.setTag('cora-cowork.backend_startup.boundary_stage', failureInfo.backendBoundaryStage);
    }
    if (failureInfo.localDataIssueKind) {
      scope.setTag('cora-cowork.backend_startup.local_data_issue_kind', failureInfo.localDataIssueKind);
    }
    if (failureInfo.incompleteInstallationKind) {
      scope.setTag('cora-cowork.backend_startup.incomplete_installation_kind', failureInfo.incompleteInstallationKind);
    }
    for (const [tag, value] of [
      ['cora-cowork.backend_startup.missing_bundled_dir', getBooleanTagValue(failureInfo.missingBundledCoraCoworkDir)],
      ['cora-cowork.backend_startup.missing_runtime_dir', getBooleanTagValue(failureInfo.missingRuntimeDir)],
      ['cora-cowork.backend_startup.missing_binary', getBooleanTagValue(failureInfo.missingBackendBinary)],
      ['cora-cowork.backend_startup.missing_hub_dir', getBooleanTagValue(failureInfo.missingHubDir)],
      ['cora-cowork.backend_startup.missing_pet_states_dir', getBooleanTagValue(failureInfo.missingPetStatesDir)],
      ['cora-cowork.backend_startup.missing_pwa_dir', getBooleanTagValue(failureInfo.missingPwaDir)],
      ['cora-cowork.backend_startup.install_path_kind', getInstallPathKind(details?.resourcesPath)],
      ['cora-cowork.backend_startup.last_update_status', getString(autoUpdateDiagnostics?.lastEvent?.status)],
      [
        'cora-cowork.backend_startup.health_polling_delayed',
        getBooleanTagValue(
          typeof details?.healthCheckPollingDelayed === 'boolean' ? details.healthCheckPollingDelayed : undefined
        ),
      ],
      ['cora-cowork.backend_startup.health_attempts_bucket', getHealthAttemptBucket(details?.healthCheckAttempts)],
      [
        'cora-cowork.backend_startup.health_attempt_deficit_bucket',
        getHealthAttemptBucket(details?.healthCheckAttemptDeficit),
      ],
      ['cora-cowork.backend_startup.health_timeout_overrun_bucket', getDurationBucket(details?.healthCheckTimeoutOverrunMs)],
      ['cora-cowork.backend_startup.health_max_attempt_gap_bucket', getDurationBucket(details?.healthCheckMaxAttemptGapMs)],
      [
        'cora-cowork.backend_startup.seconds_since_quit_and_install',
    ...
    if (details) {
      scope.setContext('cora_cowork_startup', details);
      scope.setExtra('cora_cowork_startup', details);
    }
    scope.setContext('cora_cowork_startup_classification', { ...failureInfo });
    scope.setExtra('cora_cowork_startup_classification', failureInfo);
    scope.setContext('cora_cowork_install_diagnostics', installDiagnostics);
    scope.setExtra('cora_cowork_install_diagnostics', installDiagnostics);
...
async function runStartupLogReport(): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.addAttachment({
      filename: 'cora-cowork-logs.log.gz',
      data: pack.gzipped,
