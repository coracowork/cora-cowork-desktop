import { CORA_COWORK_FILES_MARKER, CORA_COWORK_TIMESTAMP_REGEX } from '@/common/config/constants';
...
export const buildDisplayMessage = (input: string, files: string[], workspacePath: string): string => {
  ...
  const displayPaths = files.map((file_path) => {
    const sanitizedPath = file_path.replace(CORA_COWORK_TIMESTAMP_REGEX, '$1');
    ...
    if (isAbsolute) {
      ...
      if (normalizedFile.startsWith(normalizedWorkspaceWithForwardSlash + '/')) {
        ...
        return `${normalizedWorkspace}/${relativePath.replace(CORA_COWORK_TIMESTAMP_REGEX, '$1')}`;
      }
