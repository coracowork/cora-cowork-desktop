...
declare global {
  interface Window {
    __CoraSafeResizeObserver__?: boolean;
    __CoraResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__CoraSafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__CoraSafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__CoraResizeObserverPatched__) {
    ...
    window.__CoraResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__CoraResizeObserverPatched__) {
    ...
    console.__CoraResizeObserverPatched__ = true;
  }
};
