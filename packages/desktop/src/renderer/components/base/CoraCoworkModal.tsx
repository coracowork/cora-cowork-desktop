...
/** CoraCoworkModal 组件 Props */
export interface CoraCoworkModalProps extends Omit<ModalProps, 'title' | 'footer'> {
...
const STD_HEADER_CLASS = 'cora-cowork-modal-std-header flex items-start justify-between gap-16px px-24px pt-20px pb-16px';
...
const STD_BODY_LAYOUT_CLASS = 'cora-cowork-modal-std-body min-h-0 flex-1 overflow-y-auto';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <CoraCoworkModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </CoraCoworkModal>
 *
 * // 预设尺寸 / Preset size
 * <CoraCoworkModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </CoraCoworkModal>
 *
...
const CoraCoworkModal: React.FC<CoraCoworkModalProps> = ({
  children,
  variant,
  size,
  header,
  ...
  const renderFooter = () => {
    ...
    if (footerConfig.render) {
      ...
      const footerClassName = classNames(
        ...
        isStandard && 'cora-cowork-modal-std-footer',
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={classNames('cora-cowork-modal', isStandard && 'cora-cowork-modal-standard', className)}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div
        className={classNames('cora-cowork-modal-wrapper', isStandard && 'flex flex-col min-h-0')}
        style={{ borderRadius: borderRadiusVal }}
      >
        {renderHeader()}
        <div
          className={classNames(
            'cora-cowork-modal-body-content',
...
CoraCoworkModal.displayName = 'CoraCoworkModal';

export default CoraCoworkModal;
