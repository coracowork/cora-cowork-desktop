 ...
 * Legacy backend/custom-agent migration is handled by CoraCore during channel
...
export function getDefaultChannelAssistant(assistants: Assistant[]): Assistant | undefined {
  return (
    assistants.find((assistant) => assistant.source === 'generated' && isCorarsAssistant(assistant)) ||
    assistants.find((assistant) => isCorarsAssistant(assistant)) ||
    assistants[0]
  );
}
