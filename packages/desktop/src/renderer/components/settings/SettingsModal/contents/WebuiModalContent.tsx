...
const getDisplayUrl = useCallback(() => {
  ...
  if (useRemote && currentIP) {
    return `http://${currentIP}:${actualPort}`;
  }
  return `http://localhost:${actualPort}`;
}, [allowRemotePreference, getLocalIP, status?.allowRemote, status?.running, status?.port, port]);
                  ...
                  onClick={() =>
                    void talkToButler({
                      prompt: t('settings.talkToButler.prompt.setupRemote', {
                        defaultValue:
                          'Help me set up remote access so I can open CoraCowork from my phone or over the internet.',
                      }),
                    })
                  }
                >
