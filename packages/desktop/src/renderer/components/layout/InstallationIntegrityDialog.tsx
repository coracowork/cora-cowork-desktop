...
const CORA_COWORK_DOWNLOAD_URL = 'https://www.cora-cowork.com/';
...
export function openDownloadLatest(): void {
  window.open(CORA_COWORK_DOWNLOAD_URL, '_blank', 'noopener,noreferrer');
}
...
function buildInstallationIntegrityTags(diagnostics: InstallationIntegrityDiagnostics): FeedbackEventTags {
  const tags: FeedbackEventTags = {
    'cora-cowork.installation_integrity.user_report': 'true',
    'cora-cowork.installation_integrity.report_source': diagnostics.source,
  };

  if (diagnostics.runtime?.failureKind) {
    tags['cora-cowork.installation_integrity.failure_kind'] = diagnostics.runtime.failureKind;
  }
  if (diagnostics.runtime?.resource) {
    tags['cora-cowork.runtime_resource'] = diagnostics.runtime.resource;
  }
  if (diagnostics.runtime?.resourceId) {
    tags['cora-cowork.runtime_resource_id'] = diagnostics.runtime.resourceId;
  }
  if (diagnostics.runtime?.scopeKind) {
    tags['cora-cowork.runtime_scope'] = diagnostics.runtime.scopeKind;
  }

  const reason = diagnostics.backendStartupFailure?.reason;
  if (typeof reason === 'string') {
    tags['cora-cowork.backend_startup_failure.reason'] = reason;
  }
  const backendBoundaryCode = diagnostics.backendStartupFailure?.backendBoundaryCode;
  if (typeof backendBoundaryCode === 'string') {
    tags['cora-cowork.backend_startup_failure.backend_boundary_code'] = backendBoundaryCode;
  }
  const backendBoundaryStage = diagnostics.backendStartupFailure?.backendBoundaryStage;
  if (typeof backendBoundaryStage === 'string') {
    tags['cora-cowork.backend_startup_failure.backend_boundary_stage'] = backendBoundaryStage;
  }
...
export async function reportInstallationIntegrityDiagnostics(
  ...
  if (typeof window !== 'undefined' && window.__coraCoworkE2ETest) {
