...
import brandLogo from '@/renderer/assets/logo.svg';
...
const Layout: React.FC<{
  ...
  // The "CoraCowork" wordmark acts as Home / Back-to-Chat, but only from settings routes.
  ...
  const handleBrandHome = useCallback(() => {
    ...
    try {
      target = sessionStorage.getItem('cora-cowork:last-non-settings-path');
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      window.dispatchEvent(new CustomEvent('cora-cowork-open-update-modal', { detail: { source: 'tray' } }));
    };
            ...
            >
              ...
              >
                ...
                {isSettingsRoute ? (
                  ...
                  <div className='text-16px text-t-primary collapsed-hidden font-semibold'>CoraCowork</div>
                )}
                {isMobile && !collapsed && (
                  <button
                    type='button'
