...
import CorarsModelSelector from '@/renderer/pages/conversation/platforms/corars/CorarsModelSelector';
import { useCorarsModelSelection } from '@/renderer/pages/conversation/platforms/corars/useCorarsModelSelection';
...
const NON_ACP_BACKENDS = new Set(['corars', 'openclaw-gateway', 'nanobot', 'remote']);
...
/** Compact corars model selector for the agent header */
const CorarsHeaderModelSelector: React.FC<{ conversation_id: string; initialModel?: TProviderWithModel }> = ({
  conversation_id,
  initialModel,
}) => {
  ...
  const modelSelection = useCorarsModelSelection({ initialModel, onSelectModel });
...
const AssistantChatSlot: React.FC<{
  ...
  const isCorars = conversation?.type === 'corars';
    ...
    <div className='flex flex-col h-full' style={{ background: `color-mix(in srgb, ${color} 4%, var(--bg-base))` }}>
      <div className='flex items-center justify-between gap-8px px-12px h-40px shrink-0 border-b border-solid border-[color:var(--border-base)] relative z-10 bg-1'>
        ...
        <div className='flex items-center gap-8px shrink-0'>
          ...
          {!isMobile && assistant.conversation_id && !isCorars && isAcpLike && (
          ...
          {!isMobile && isCorars && assistant.conversation_id && (
            <div className='min-w-0 max-w-140px [&_button]:max-w-full [&_button_span]:truncate'>
              <CorarsHeaderModelSelector
                key={assistant.conversation_id}
                conversation_id={assistant.conversation_id}
