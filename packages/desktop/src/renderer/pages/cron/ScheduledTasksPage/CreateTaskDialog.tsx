...
import CoraCoworkModal from '@renderer/components/base/CoraCoworkModal';
...
import { assistantRuntimeKey, isCorarsAssistant } from '@/common/types/agent/assistantTypes';
...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  const isGeminiMode = resolvedBackend === 'gemini' || resolvedBackend === 'corars';

  // Providers compatible with corars (Cora CLI does not support Google Auth).
  // Computed independent of the current selection so assistant options backed
  // by corars can be disabled when no provider is configured.
  const corarsProviders = useMemo(
    () => providers.filter((p) => !p.platform?.toLowerCase().includes('gemini-with-google-auth')),
    [providers]
  );
  const hasCorarsProvider = corarsProviders.length > 0;

  const filteredProviders = useMemo(
    () => (resolvedBackend === 'corars' ? corarsProviders : providers),
    [resolvedBackend, providers, corarsProviders]
  );

  // Build Gemini current_model from model_id for GuidModelSelector.
  // For corars edit mode, prefer the exact provider_id stored in model —
  ...
  const geminiCurrentModel = useMemo<TProviderWithModel | undefined>(() => {
    if (resolvedBackend !== 'corars' || !model_id) return undefined;

    const editedProviderId =
      resolvedBackend === 'corars' ? editJob?.metadata.agent_config?.model?.provider_id : undefined;
    if (editedProviderId) {
  ...
  const acpCachedModelInfo = useMemo<AcpModelInfo | null>(() => {
    if (!resolvedBackend || resolvedBackend === 'gemini' || resolvedBackend === 'corars') return null;
    return buildAssistantModelInfo(selectedAssistantModels);
  }, [resolvedBackend, selectedAssistantModels]);

  // Auto-pick the first available model from /api/providers when corars is
  // selected but none is set yet. Source of truth is the backend provider
  // list — do NOT read from any frontend-cached default.
  useEffect(() => {
    if (resolvedBackend !== 'corars' || model_id) return;
    for (const provider of corarsProviders) {
  ...
  }, [resolvedBackend, model_id, corarsProviders, getAvailableModels]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      let agent_config: ICreateCronJobParams['agent_config'];
      if (canEditAgentConfig) {
        ...
        agent_config = resolveCronAgentConfig({
          ...
          presetAssistants,
          selectedCorarsProvider: geminiCurrentModel
            ? {
          ...
          corarsModelRequiredMessage: t('cron.page.form.corarsModelRequired'),
        }).agent_config;
      }
      ...
      <div>
        <Form form={form} layout='vertical'>
          ...
          >
            ...
            >
              {presetAssistants.map((assistant) => {
                ...
                const avatar = resolveAssistantAvatar(assistant.avatar);
                const disabled = isCorarsAssistant(assistant) && !hasCorarsProvider;
                return (
                  <Option key={assistant.id} value={assistant.id} disabled={disabled}>
                    <div
                      className='flex items-center gap-8px'
                      title={disabled ? t('cron.page.form.corarsNoProvider') : undefined}
                    >
                      {avatar.kind === 'image' ? (
                        <img src={avatar.value} alt={name} className='w-16px h-16px object-contain' />
                      ) : avatar.kind === 'emoji' ? (
                      ...
                      <span>{name}</span>
                      {disabled && (
                        <span className='text-12px text-t-tertiary'>{t('cron.page.form.corarsNoProvider')}</span>
                      )}
                    </div>
                  </Option>
                );
          ...
          <FormItem label={t('cron.page.form.executionMode')}>
            ...
            <div className='mt-10px rounded-12px border border-solid border-(--color-border-2) bg-fill-2 px-14px py-12px'>
