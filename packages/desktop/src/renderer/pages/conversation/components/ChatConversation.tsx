...
type CorarsConversation = Extract<TChatConversation, { type: 'corars' }>;

const CorarsConversationPanel: React.FC<{ conversation: CorarsConversation; sliderTitle: React.ReactNode }> = ({
  conversation,
  sliderTitle,
}) => {
  ...
  const corarsAssistantId = presetAssistantInfo?.assistantId;
  ...
  const chatLayoutProps = {
    ...
    backend: 'corars' as const,
    presetAssistant: presetAssistantInfo ? { ...presetAssistantInfo, id: corarsAssistantId } : undefined,
  };
...
const ChatConversation: React.FC<{
  ...
  const isCorarsConversation = conversation?.type === 'corars';
  ...
  const acpConversation = isCorarsConversation ? undefined : conversation;
  ...
  const conversationNode = useMemo(() => {
    if (!conversation || isCorarsConversation) return null;
  ...
  const modelSelector = useMemo(() => {
    if (!conversation || isCorarsConversation) return undefined;
  ...
  }, [conversation, isCorarsConversation, isMobile, isLegacyReadOnlyConversation, resolvedConversationBackend]);

  if (conversation && conversation.type === 'corars') {
    return <CorarsConversationPanel key={conversation.id} conversation={conversation} sliderTitle={sliderTitle} />;
  }
