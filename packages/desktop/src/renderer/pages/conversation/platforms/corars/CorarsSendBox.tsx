...
const CoraCoworkrsSendBox: React.FC<{
  ...
  useEffect(() => {
    ...
    const storageKey = `corars_initial_message_${conversation_id}`;
    const processedKey = `corars_initial_processed_${conversation_id}`;
  ...
  const onSendHandler = async (message: string) => {
    ...
    clearFiles();
    emitter.emit('corars.selected.file.clear');
  ...
  useAddEventListener('corars.selected.file', setAtPath);
  useAddEventListener('corars.selected.file.append', (selectedItems: Array<string | FileOrFolderItem>) => {
    ...
    <div className={`${sendBoxWidthClass} flex flex-col mt-auto mb-16px`}>
      ...
      <SendBox
        ...
        onSelectedWorkspaceItemsChange={(items) => {
          emitter.emit('corars.selected.file', items);
          setAtPath(items);
        }}
            ...
            {uploadFile.length > 0 && (
              <HorizontalFileList>
                {uploadFile.map((path) => (
                  <FilePreview
                    key={path}
                    data-testid={`corars-file-tag-${uploadFile.indexOf(path)}`}
                    path={path}
                ...
                {atPath.map((item) => {
                  ...
                  if (!item.isFile) {
                    ...
                    return (
                      <Tag
                        key={item.path}
                        data-testid={`corars-folder-tag-${folderIndex}`}
                        color='blue'
                        closable
                        onClose={() => {
                          const newAtPath = atPath.filter((v) => (typeof v === 'string' ? true : v.path !== item.path));
                          emitter.emit('corars.selected.file', newAtPath);
                          setAtPath(newAtPath);
                        }}
                      >
                        {item.name}
