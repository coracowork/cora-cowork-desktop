...
import { assistantRuntimeKey, isCorarsAssistant, type Assistant } from '@/common/types/agent/assistantTypes';
...
export function pickDefaultAssistantSelectionKey(assistants: Assistant[]): string | null {
  const enabledAssistants = assistants.filter((assistant) => assistant.enabled !== false);
  const preferred =
    enabledAssistants.find((assistant) => assistant.source === 'generated' && isCorarsAssistant(assistant)) ??
    enabledAssistants.find((assistant) => isCorarsAssistant(assistant)) ??
    enabledAssistants[0];
