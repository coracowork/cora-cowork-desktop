...
/**
 * Hook that manages the send logic for ACP and CoraCowork CLI conversations.
 */
  ...
  const handleSend = useCallback(async () => {
    ...
    if (assistantBackend === 'corars') {
      ...
      try {
        ...
        sessionStorage.setItem(`corars_initial_message_${conversation.id}`, JSON.stringify(initialMessage));

        await navigate(`/conversation/${conversation.id}`);
      } catch (error: unknown) {
        console.error('Failed to create CoraCowork CLI conversation:', error);
        throw error;
      }
      return;
    }
