...
const GuidPage: React.FC = () => {
  ...
  // --- Hooks ---
  // Only corars uses this provider-based model picker now (Gemini runs as a
  // regular ACP backend with its own model selector).
  const modelSelection = useGuidModelSelection('corars');
  ...
  useEffect(() => {
    ...
    const signature = JSON.stringify({
      ...
      availableModels: {
        ...
        corars: modelSelection.modelList.map((provider) => ({
          id: provider.id,
    ...
    const applyAssistantDefaults = async () => {
      ...
      if (shouldApplyDefaultModel && effectiveBackend === 'corars') {
  ...
  // Only corars now — Gemini runs as a regular ACP backend with ACP-cached models.
  const PROVIDER_BASED_AGENTS = new Set(['corars']);
