...
const CORA_COWORK_HUB_REPO_URL = 'https://github.com/coracowork/CoraHub';
...
export const AgentHubModal: React.FC<AgentHubModalProps> = ({ visible, onCancel }) => {
  ...
  const openCoraHubRepo = () => {
    void openExternalUrl(CORA_COWORK_HUB_REPO_URL).catch(console.error);
  };
    ...
    >
      <div>
        <div className='mb-12px flex flex-wrap items-center justify-start gap-x-6px gap-y-2px text-left'>
          ...
          <Link className='text-12px leading-18px' onClick={openCoraHubRepo}>
            {t('settings.agentManagement.marketContributionAction', {
              defaultValue: 'Open a PR on CoraHub',
            })}
          </Link>
        </div>
