...
/** Backend manifest id of the built-in CoraCowork Butler assistant. */
const BUTLER_ASSISTANT_ID = 'cora-cowork-assistant';
 ...
 * select the CoraCowork Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the CoraCowork Butler for you' })
            );
          }
        }
      } catch (error) {
