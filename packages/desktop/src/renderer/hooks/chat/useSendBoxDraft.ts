...
const setDraft = <K extends DraftConversationType>(
  ...
  switch (type) {
    ...
    case 'corars':
      if (draft) {
        store.corars.set(conversation_id, draft as Extract<Draft, { _type: 'corars' }>);
      } else {
        store.corars.delete(conversation_id);
      }
      break;
    default:
      break;
...
const getDraft = <K extends DraftConversationType>(
  ...
  switch (type) {
    ...
    case 'corars':
      return store.corars.get(conversation_id) as Extract<Draft, { _type: K }>;
    default:
      return undefined;
  }
};
