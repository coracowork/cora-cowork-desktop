...
import { BackendLifecycleManager } from '@cora-cowork/web-host';
...
const isE2ETestMode = process.env.CORA_COWORK_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.CORA_COWORK_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[CoraCowork] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[CoraCowork] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
let shouldRestoreDesktopWebUIFromPreferences = false;
...
ipcMain.on('get-backend-port', (event) => {
  ...
  const fallbackPort = (globalThis as typeof globalThis & { __backendPort?: number }).__backendPort;
  event.returnValue = fallbackPort ?? 0;
});
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      method: 'POST',
      headers: {
        'x-cora-cowork-internal': '1',
      },
    }).catch((error) => {
      console.error('[CoraCowork] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[CoraCowork] runBackendMigrations completed');
    } catch (error) {
      console.error('[CoraCowork] Backend migration hook threw:', error);
    }
  })();
};
...
function scheduleRestoreDesktopWebUIFromPreferences(): void {
  if (isE2ETestMode || isWebUIMode || isResetPasswordMode) return;
  if (appReadyDone) {
    void restoreDesktopWebUIFromPreferences().catch((error) => {
      console.error('[WebUI] Failed to auto-restore:', error);
    });
    return;
  }

  shouldRestoreDesktopWebUIFromPreferences = true;
}

function markBackendReady(backendPort: number, source: string): void {
  if (backendStartedOk) return;
  console.log(`[CoraCowork] ${source} ready (port=${backendPort})`);
  ...
  scheduleRestoreDesktopWebUIFromPreferences();
}

function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  const reason = process.env.CORA_COWORK_DEBUG_BACKEND_STARTUP_FAILURE as BackendStartupFailureInfo['reason'] | undefined;
  if (!reason) {
    return null;
  }
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[CoraCowork] Ignoring CORA_COWORK_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[CoraCowork] Ignoring unknown CORA_COWORK_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[CoraCowork] Creating main window...');
  ...
  console.log(`[CoraCowork] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[CoraCowork] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[CoraCowork] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[CoraCowork] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
      // Send the current backend port to the renderer — the preload may have
      // captured 0 if the backend hadn't started by then.
      const backendPort = (globalThis as typeof globalThis & { __backendPort?: number }).__backendPort;
      if (backendPort && backendPort > 0) {
        mainWindow.webContents.send('backend-port-update', backendPort);
      }
    });
  ...
  const disableAutoUpdater =
    process.env.CORA_COWORK_DISABLE_AUTO_UPDATE === '1' || process.env.CORA_COWORK_E2E_TEST === '1' || isCiRuntime;
  if (!disableAutoUpdater) {
    ...
    console.log('[CoraCowork] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[CoraCowork] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[CoraCowork] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[CoraCowork] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[CoraCowork] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[CoraCowork] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[CoraCowork] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[CoraCowork] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[CoraCowork] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[CoraCowork] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[CoraCowork] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[CoraCowork] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[CoraCowork] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[CoraCowork:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  if (debugBackendStartupFailure) {
    ...
    // Start CoraCore only after initializeProcess(). initStorage may open
    ...
    exposeBackendPort(backendPort);
    console.log(`[CoraCowork] Backend port set to ${backendPort} (onStarted)`);
  ...
  logDir: sysDir.logDir, // 🔥 Passa o diretório de logs
});
  ...
  try {
    ...
    console.error('[CoraCowork] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[CoraCowork] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`CoraCowork --webui` or packaged `cora-cowork-web` mode that
      ...
      // M6: Switch to @cora-cowork/web-host
      const handle = await startWebHost({
        ...
        // Expose the same CORA_COWORK_{CACHE,WORK,LOG}_DIR env the desktop IPC path
    ...
    appReadyDone = true;
    mark('createWindow');
    if (shouldRestoreDesktopWebUIFromPreferences) {
      shouldRestoreDesktopWebUIFromPreferences = false;
      void restoreDesktopWebUIFromPreferences().catch((error) => {
        console.error('[WebUI] Failed to auto-restore:', error);
      });
    }
    ...
    if (!isE2ETestMode) {
      // WebUI restore is now scheduled when the backend is marked ready.
    }
...
// Register cora-cowork:// as the default protocol client
...
// macOS: handle cora-cowork:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[CoraCowork] App initialization failed:', error);
    app.quit();
  });
...
installQuitCleanup({
  ...
  // Stop CoraCore subprocess — backend shutdown kills all agent children
...
app.on('will-quit', () => {
  console.log('[CoraCowork] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[CoraCowork] quit (exitCode=${exitCode})`);
});
