 ...
 * calls routed to coracore. Electron-native operations (window controls,
...
export const conversation = {
  ...
  createWithConversation: withResponseMap(
    httpPost<TChatConversation, { conversation: TChatConversation }>('/api/conversations/clone', (p) => {
      const isCorars = p.conversation.type === 'corars';
...
export const application = {
  ...
  // The backend reads CORA_COWORK_*_DIR env vars on boot, so it does not own this config.
...
// ---------------------------------------------------------------------------
// Google subscription status (Google OAuth provider path, used by corars)
// ---------------------------------------------------------------------------
