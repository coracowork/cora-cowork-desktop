...
describe('Windows bundled coracore install verifier', () => {
  ...
  runOnWindows('fails an old-version-only Codex ACP install directory', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'cora-cowork-install-verify-'));
    const installDir = join(tmp, 'install');
    const managedRoot = join(installDir, 'resources', 'bundled-coracore', 'win32-x64', 'managed-resources');
    const logPath = join(tmp, 'verify.log');

    try {
      writeFile(join(installDir, 'resources', 'bundled-coracore', 'win32-x64', 'coracore.exe'), 'x');
      writeJson(join(installDir, 'resources', 'bundled-coracore', 'win32-x64', 'manifest.json'), {
        platform: 'win32',
        arch: 'x64',
      });
