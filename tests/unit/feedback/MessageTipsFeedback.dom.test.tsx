...
describe('agent error locale copy', () => {
  ...
  it('does not label app-side errors as direct CoraCowork ownership', () => {
    const localeDir = path.join(process.cwd(), 'packages/desktop/src/renderer/services/i18n/locales');
    ...
    for (const localeName of supportedLocaleNames) {
      ...
      expect(agentError.ownership.coracowork, localeName).not.toMatch(/CoraCowork/);

      for (const [code, copy] of Object.entries<Record<string, string>>(agentError.codes)) {
        if (!code.startsWith('CORA_COWORK_')) continue;

        expect(copy.title, `${localeName} ${code} title`).not.toMatch(/CoraCowork/);
        expect(copy.body, `${localeName} ${code} body`).not.toMatch(/CoraCowork/);
      }
    }
  });
