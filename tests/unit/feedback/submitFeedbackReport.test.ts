...
describe('submitFeedbackReport', () => {
  ...
  it('submits a user-feedback event with tags, extra context, logs, and attachments', async () => {
    const collectFeedbackLogs = vi.fn().mockResolvedValue({
      filename: 'coracowork-logs.log.gz',
      data: [1, 2, 3],
    });
    ...
    await submitFeedbackReport({
      ...
      collectLogs: true,
      description: '  CoraCore   cannot start  ',
      extra: {
      ...
      moduleLabel: 'CoraCowork installation is incomplete',
      tags: {
        'coracowork.installation_integrity.report_source': 'backend_startup_failure',
      },
    });
    ...
    expect(sentryMocks.setTag).toHaveBeenCalledWith(
      'coracowork.installation_integrity.report_source',
      'backend_startup_failure'
    );
    expect(sentryMocks.captureEvent).toHaveBeenCalledWith(
      {
        level: 'info',
        message: 'CoraCowork installation is incomplete: CoraCore cannot start',
        extra: {
          description: 'CoraCore cannot start',
          installation_integrity: {
            source: 'backend_startup_failure',
