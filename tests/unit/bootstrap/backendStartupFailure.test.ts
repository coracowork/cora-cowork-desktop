...
describe('classifyBackendStartupFailure', () => {
  ...
  it('classifies missing startup directory preparation as a startup directory failure', () => {
    const error = new Error('CoraCore startup directory preparation failed') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'spawn',
      workDir: 'D:\\ai\\CoraCowork\\工作目录',
      causeMessage: 'ENOENT: no such file or directory, mkdir D:\\ai\\CoraCowork\\工作目录',
    };
  ...
  it('classifies startup directory permission failures separately from incomplete installs', () => {
    ...
    error.details = {
      stage: 'spawn',
      workDir: 'D:\\ai\\CoraCowork\\工作目录',
      causeMessage: 'EPERM: operation not permitted, mkdir D:\\ai\\CoraCowork\\工作目录',
    };
  ...
  it('does not classify post-resolution binary spawn ENOENT as a startup directory failure', () => {
    const error = new Error('CoraCore process emitted an error before startup') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'spawn_error',
      binaryPath: 'D:\\apps\\CoraCowork\\resources\\bundled-CoraCore\\win32-x64\\Corars.exe',
      causeMessage: 'spawn D:\\apps\\CoraCowork\\resources\\bundled-CoraCore\\win32-x64\\Corars.exe ENOENT',
    };
  ...
  it('classifies recoverable database corruption boundary failures separately from data migration failures', () => {
    ...
    error.details = {
      ...
      stderrTail:
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.recoverable_corruption databasePath=/db/coracowork-backend.db: failed to initialize application data',
    };
  ...
  it('classifies packaged macOS architecture mismatches separately from generic startup failures', () => {
    const error = new Error('CoraCowork package architecture does not match this Mac') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
