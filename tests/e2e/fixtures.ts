 ...
 *      (e.g. out/linux-unpacked/coracowork, out/mac-arm64/CoraCowork.app, out/win-unpacked/CoraCowork.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coracowork-e2e-state-'));
const e2eStateFile = path.join(e2eStateSandboxDir, 'extension-states.json');
// Disposable userData root so CoraCore migrates a fresh DB per run instead of
...
const e2eUserDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coracowork-e2e-userdata-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/CoraCowork.exe  or  out/win-x64-unpacked/CoraCowork.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'CoraCowork.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/CoraCowork.app/Contents/MacOS/CoraCowork  or  out/mac/CoraCowork.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'CoraCowork');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/coracowork  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['coracowork', 'CoraCowork']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    CORA_COWORK_EXTENSIONS_PATH: process.env.CORA_COWORK_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    CORA_COWORK_EXTENSION_STATES_FILE: process.env.CORA_COWORK_EXTENSION_STATES_FILE || e2eStateFile,
    CORA_COWORK_DISABLE_AUTO_UPDATE: '1',
    CORA_COWORK_DISABLE_DEVTOOLS: '1',
    CORA_COWORK_E2E_TEST: '1',
    CORA_COWORK_E2E_USER_DATA_DIR: process.env.CORA_COWORK_E2E_USER_DATA_DIR || e2eUserDataDir,
    CORA_COWORK_CDP_PORT: '0',
  };
