# Cora CLI (Corars) E2E 测试用例

**版本**: Gate 2 初稿
**作者**: chat-Corars-designer
**日期**: 2026-04-22
**状态**: 待审核
...
1. **Corars binary 可用**：通过 `ipcBridge.fs.findCorarsBinary.invoke()` 验证，否则 skip 全部测试
2. **用户配置的模型列表至少 1 个可用 provider**：
   ...
   - 若无可用 provider：skip 全部 Corars 测试，原因："No non-Google-Auth provider with apiKey configured"

3. **测试数据准备**：临时工作目录 `/tmp/e2e-Corars-<timestamp>/`

**模型获取 helper**（engineer 实现）:

```typescript
// tests/e2e/helpers/chatCorars.ts
...
| 维度       | 可选值                     | 说明                                                                                               |
| ---------- | -------------------------- | -------------------------------------------------------------------------------------------------- |
| 关联文件夹 | 无 / 单 / 多               | `atPath` 数组（源码：`CorarsSendBox.tsx:331-337`）                                                 |
...
| 权限       | default / auto_edit / yolo | 来源：Corars runtime capabilities                                                                  |
| 对话中操作 | 切换模型 / 切换权限 / 无   | 对话页 `CorarsModelSelector` / `AgentModeSelector`                                                 |

### 1.4 清理约定

**命名模式**: 所有测试对话命名为 `E2E-Corars-<timestamp>-<scenario>`

**清理顺序**（每个用例 `afterEach` 执行）:

1. 停止 binary 进程：`ipcBridge.conversation.stopAgent.invoke(conversationId)`
2. 删除 DB 记录：`DELETE FROM conversations WHERE name LIKE 'E2E-Corars-%'`（级联删除 messages）
3. 删除临时目录：`fs.rm('/tmp/e2e-Corars-*', { recursive: true })`
4. 清理 sessionStorage：`sessionStorage.removeItem('Corars_initial_message_*')` + `sessionStorage.removeItem('Corars_initial_processed_*')`
...
- 同 §1.2 全局前置条件
- 调用 `getCorarsTestModels(page)` 获取 `modelA`（默认模型）
...
2. 选择 Corars agent（点击 `[data-agent-backend="Corars"]`）
3. 确认权限选择器显示 `default`（`AgentModeSelector` 默认值）
4. 输入测试消息："Hello, Corars! Please list files in current directory."
5. 点击发送按钮
6. 等待跳转至对话页（URL 匹配 `/conversation/Corars/*`）
...
FROM conversations
WHERE name LIKE 'E2E-Corars-%' AND id = ?;
-- 期望: type='Corars', mode='default', status='finished'

-- 2. 验证用户消息
SELECT id, type, position, content, status
FROM messages
WHERE conversation_id = ? AND position = 'right';
-- 期望: type='text', position='right', json_extract(content, '$.content') 包含 "Hello, Corars!"
...
- conversations name: `E2E-Corars-<timestamp>-minimal-path`
- temp dir: `/tmp/e2e-Corars-<timestamp>-tc-a-01/`（workspace）
- sessionStorage: `Corars_initial_message_${conversationId}`, `Corars_initial_processed_${conversationId}`
...
- 同 §1.2 全局前置条件
- 临时工作目录存在测试文件夹：`/tmp/e2e-Corars-<timestamp>/test-folder/`
...
1. 创建临时目录 + 测试文件夹：
   ```bash
   mkdir -p /tmp/e2e-Corars-<timestamp>/test-folder/
   echo "sample content" > /tmp/e2e-Corars-<timestamp>/test-folder/sample.txt
   ```
2. 打开 guid 页，选择 Corars agent
3. 从文件树选择 `test-folder/`（触发 `emitter.emit('Corars.selected.file', [{ path, name, isFile: false }])`）
...
- conversations name: `E2E-Corars-<timestamp>-folder-single`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`（包含 test-folder）
...
- 同 §1.2 全局前置条件
- 测试文件存在：`/tmp/e2e-test-file.txt`（内容："Test file content for Corars E2E"）
...
1. 创建测试文件：
   ```bash
   echo "Test file content for Corars E2E" > /tmp/e2e-test-file.txt
   ```
2. 打开 guid 页，选择 Corars agent
3. 上传文件：
...
- conversations name: `E2E-Corars-<timestamp>-file-single`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`（workspace）
...
- 同 §1.2 全局前置条件
- 调用 `getCorarsTestModels(page)` 获取 `{ modelA, modelB }`
...
- conversations name: `E2E-Corars-<timestamp>-model-second`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
1. 打开 guid 页，选择 Corars agent
2. 打开权限选择器（`AgentModeSelector`，`data-testid="agent-mode-selector-Corars"`）
...
- conversations name: `E2E-Corars-<timestamp>-yolo-mode`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`（包含 binary 创建的 test.txt）
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-Corars-<timestamp>-auto-edit-mode`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
2. 等待首条 AI 回复完成
3. 点击对话页模型选择器（`CorarsModelSelector`，`data-testid="Corars-model-selector"`）
...
- conversations name: `E2E-Corars-<timestamp>-switch-model`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-Corars-<timestamp>-switch-permission-1`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-Corars-<timestamp>-switch-permission-2`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
1. 创建测试数据：
   ```bash
   mkdir -p /tmp/e2e-Corars-<timestamp>/folder-a/
   echo "content A" > /tmp/e2e-Corars-<timestamp>/folder-a/file-a.txt
...
- conversations name: `E2E-Corars-<timestamp>-combo-folder-file-model`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`
...
- conversations name: `E2E-Corars-<timestamp>-multi-files`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`
...
1. 创建测试数据：
   ```bash
   mkdir -p /tmp/e2e-Corars-<timestamp>/folder-x/ /tmp/e2e-Corars-<timestamp>/folder-y/
   echo "X content" > /tmp/e2e-Corars-<timestamp>/folder-x/x.txt
   echo "Y content" > /tmp/e2e-Corars-<timestamp>/folder-y/y.txt
   ```
2. 打开 guid 页，选择 Corars agent
...
- conversations name: `E2E-Corars-<timestamp>-multi-folders`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`（包含 folder-x, folder-y）
- sessionStorage: 同 TC-A-01
...
- 控制台输出 skip 原因："Corars binary not found, skipping E2E tests"
...
export async function checkCorarsBinary(): Promise<boolean> {
  try {
...
test.beforeAll(async () => {
  const hasBinary = await checkCorarsBinary();
  if (!hasBinary) {
...
2. 手动触发文件夹选择事件（模拟选择不存在的路径）：
   ```typescript
   emitter.emit('Corars.selected.file', [
...
- conversations name: `E2E-Corars-<timestamp>-nonexistent-folder`
- temp dir: `/tmp/e2e-Corars-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
4. **并发对话场景**（暂缓至后续 Gate）：
   - 场景：同时打开 2 个 Corars 对话，轮流发送消息 → 验证 binary 进程隔离
...
1. **chat-Corars-engineer** review 本用例设计，评估实现工作量
...
| `src/renderer/pages/conversation/platforms/Corars/CorarsSendBox.tsx`          | 331-337  | `atPath` 状态（关联文件夹数组）                |
| `src/renderer/pages/conversation/platforms/Corars/CorarsSendBox.tsx`          | 103-125  | 文件上传 handler                               |
| `src/renderer/pages/conversation/platforms/Corars/CorarsSendBox.tsx`          | 206-212  | 发送消息（传递 `files` 参数）                  |
| `src/renderer/pages/conversation/platforms/Corars/useCorarsModelSelection.ts` | 36-40    | 过滤 Google Auth 模型                          |
| `src/renderer/pages/conversation/platforms/Corars/CorarsSendBox.tsx`          | —        | Corars runtime capabilities 转换为权限选项     |
| `src/process/task/CorarsManager.ts`                                           | 250-259  | 权限模式自动批准逻辑                           |
| `src/process/task/CorarsManager.ts`                                           | 727-737  | `setMode()` 持久化                             |
| `src/process/task/CorarsManager.ts`                                           | 452-489  | missing finish fallback（15s 超时）            |
| `CoraCore coracowork.db`                                                          | —        | conversations + messages 由 backend 独占持久化 |
...
-- 清理所有 E2E 数据
DELETE FROM conversations WHERE name LIKE 'E2E-Corars-%';
```
