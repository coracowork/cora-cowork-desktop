...
- ✅ SQLite 数据库独立(`cora-cowork.db`)
- ✅ 扩展路径指向示例目录(`CORA_COWORK_EXTENSIONS_PATH=examples/`)
...
// Returns:
// {
//   userSkillsDir: '~/.cora-cowork/skills/',     // My Skills 存储目录
...
- `/usr/local/share/cora-skills` — 系统级共享路径
...
// 创建临时外部源
const tempExternal = fs.mkdtempSync(path.join(os.tmpdir(), 'cora-cowork-e2e-external-'));
...
- E2E 框架已设置 `CORA_COWORK_EXTENSIONS_PATH=examples/`(`fixtures.ts:112`)
...
```typescript
// Setup: 创建临时目录
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cora-cowork-e2e-custom-'));
...
| `auto-skill-card-${normalizeTestId(skill.name)}`      | 自动注入技能卡片            | line ~520 `<div className="skill-card">`        | `auto-skill-card-cora-cowork-skills`       |
...
- 共享同一个 SQLite 数据库(`cora-cowork.db`)
- 并行会导致写锁冲突 + 测试数据污染
