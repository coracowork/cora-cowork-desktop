...
```typescript
return path.join(getDataPath(), 'cora-cowork.db');
```

**userData directory** (`src/process/utils/configureChromium.ts:18-26`):

- Dev mode: `~/Library/Application Support/CoraCowork-Dev/` (macOS)
- Database: `{userData}/config/cora-cowork.db`
- Shared by all E2E tests

**Conflict scenario**: If Assistant tests and Skills tests run in parallel workers:

1. Both access same `cora-cowork.db` file
...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cora-cowork-e2e-state-'));
...
**Critical bottleneck**: `workers: 1` enforced + shared `cora-cowork.db` → parallel execution impossible without refactoring.
...
1. Extend `CORA_COWORK_E2E_TEST` to include worker ID:
   ```typescript
   CORA_COWORK_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.CORA_COWORK_E2E_TEST_WORKER_ID || '0';
   return `cora-cowork-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/cora-cowork-E2E-Worker-0/config/cora-cowork.db`
   - `~/Library/Application Support/cora-cowork-E2E-Worker-1/config/cora-cowork.db`
...
**Complexity**: High (requires main process to read `CORA_COWORK_DATABASE_PATH`, conflicts with userData convention)
