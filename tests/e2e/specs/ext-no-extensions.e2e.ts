...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coracowork-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coracowork-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      CORA_COWORK_EXTENSIONS_PATH: emptyExtensionsDir,
      CORA_COWORK_EXTENSION_STATES_FILE: extensionStatesFile,
