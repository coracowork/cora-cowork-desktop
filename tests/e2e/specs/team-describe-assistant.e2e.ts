...
/** Backend /api/teams/:id GET response shape — aligns with CoraCore schema. */
  ...
  test('describes a preset and then spawns it as a teammate via the real TCP bridge', async ({ page }) => {
    ...
    try {
      ...
      try {
        ...
        expect(describeText).toContain('Backend: corars');
        ...
        expect(spawned!.assistant_backend ?? spawned!.backend).toBe('corars'); // preset backend wins
      } finally {
        client.close();
      }
    } finally {
