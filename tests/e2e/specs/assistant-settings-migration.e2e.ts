 ...
 * 1. seed a legacy `cora-cowork.db` using the pre-unification assistants schema
...
function resolveBackendBinary(): string {
  ...
  const candidates = [
    process.env.CORA_COWORK_BACKEND_BINARY,
    path.join(projectRoot, '../coraCore/target/debug/coracore'),
  ...
  throw new Error('coracore binary not found for migration e2e');
}

function schemaPath(): string {
  return path.join(process.cwd(), '../coraCore/crates/cora-cowork-db/migrations/001_initial_schema.sql');
}
...
function seedLegacyDatabase(dataDir: string): void {
  const legacyDbPath = path.join(dataDir, 'cora-cowork.db');
...
test.describe('Assistant Settings Migration', () => {
  ...
  async function startBackend(): Promise<void> {
    ...
    backend = spawn(binary, ['--local', '--port', String(MIGRATION_BACKEND_PORT), '--data-dir', dataDir], {
      ...
      env: {
        ...process.env,
        RUST_LOG: 'warn',
        CORA_COWORK_EXTENSIONS_PATH: path.join(process.cwd(), 'examples'),
      },
    });
    try {
  ...
  test.beforeEach(async () => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cora-cowork-assistant-migration-'));
  ...
  test('legacy user assistant migrates into phase-1 defaults without losing existing config', async () => {
    ...
    expect(migrated?.preset_agent_type).toBe('corars');
    ...
    expect(detail.engine.agent_backend).toBe('corars');
