 ...
 * so seeding a legacy `coracowork-config.txt` before launch and observing
...
function querySqliteIds(dataDir: string, sql: string): string[] {
  const dbPath = path.join(dataDir, 'cora-cowork.db');
...
function resolveBackendBinary(): string {
  const candidates = [process.env.CORA_COWORK_BACKEND_BINARY, path.join(os.homedir(), '.cargo', 'bin', 'coracore')].filter(
    (x): x is string => typeof x === 'string' && x.length > 0
  );
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error(`coracore binary not found. Set CORA_COWORK_BACKEND_BINARY or install to ~/.cargo/bin/coracore.`);
}
...
test.describe('Assistant User Data Migration (T5)', () => {
  ...
  test.describe('Migration contract (sibling backend)', () => {
    ...
    test.beforeEach(async () => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coracowork-e2e-migrate-'));
      await startBackend();
    });
