 ...
 */
const REMOVED_AUTO_INJECT_NAME = 'coracowork-skills';
const REMOVED_AUTO_INJECT_DIR_NAME = 'coracowork-skills';
...
function resolveBackendBinary(): string {
  const candidates = [process.env.CORA_COWORK_BACKEND_BIN, path.join(os.homedir(), '.cargo', 'bin', 'CoraCore')].filter(
    (x): x is string => typeof x === 'string' && x.length > 0
  );
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error('CoraCore binary not found. Set CORA_COWORK_BACKEND_BIN or install to ~/.cargo/bin/CoraCore.');
}
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  test('S7: builtin skills in /api/skills expose an absolute, readable location for export', async ({ page }) => {
    ...
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coracowork-e2e-s7-export-'));
    try {
  ...
  //
  // Run against a sibling `CoraCore` process on port 25903 against a
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    async function startBackend(): Promise<void> {
      ...
      const logPath = path.join(dataDir, 'sibling-CoraCore.log');
      const logFd = fs.openSync(logPath, 'a');
      const parentEnv = { ...process.env };
      // Scrub any env vars that would leak main-Electron backend state.
      delete parentEnv.CORA_COWORK_EXTENSIONS_PATH;
      delete parentEnv.CORA_COWORK_EXTENSION_STATES_FILE;
      delete parentEnv.CORA_COWORK_E2E_TEST;
      delete parentEnv.CORA_COWORK_CDP_PORT;
      delete parentEnv.CORA_COWORK_BUILTIN_SKILLS_PATH;
    ...
    test.beforeEach(() => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coracowork-e2e-builtin-skill-'));
    });
    ...
    test('S8: legacy {cacheDir}/builtin-skills/ is gone after the current Electron boot', async () => {
      ...
      // `~/.coracowork-config` tree. Failing that we at least assert the
      ...
      const candidates = [
        path.join(os.homedir(), '.coracowork-config', 'builtin-skills'),
        path.join(os.homedir(), '.coracowork-config-dev', 'builtin-skills'),
      ];
      ...
      test.info().annotations.push({
        ...
        description:
          survivors.length === 0
            ? 'no legacy builtin-skills cache dirs detected under ~/.coracowork-config*'
