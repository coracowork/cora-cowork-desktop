 ...
 * CoraCore binary using the user-provided fixtures
 * (`/Users/zhoukai/Downloads/coracowork-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `coracowork-config.txt` exactly as ConfigStorage would.
 *   2. Stage `Archive/*.md` as `<userData>/config/assistants/<id>.<locale>.md`.
 *   3. Spawn a real CoraCore bound to a throw-away data-dir.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/coracowork-config.txt';
...
function resolveBackendBinary(): string {
  const candidates = [
    process.env.CORA_COWORK_BACKEND_BIN,
    path.join(os.homedir(), '.cargo', 'bin', 'CoraCore'),
    path.resolve(__dirname, '../../../CoraCore/target/debug/CoraCore'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (existsSync(c)) return c;
  }
  throw new Error('CoraCore binary not found (set CORA_COWORK_BACKEND_BIN or build it)');
}
...
/**
 * Decode an `coracowork-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'coracowork-migrate-fixture-'));
