 ...
 *   2. No webui running → spawn a short-lived CoraCore against the same
 ...
 *   CORA_COWORK_DATA_DIR=/x bun run resetpass
 ...
 * See the comment there for why the default is `~/.cora-cowork-web*` (not `~/.cora-cowork*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `CORA_COWORK_PORT` to match
 ...
 * pass --port / CORA_COWORK_PORT if they launched webui on a non-default one.
 */
