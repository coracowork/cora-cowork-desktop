param(
  ...
  [string]$WorktreeRoot = (Join-Path $PSScriptRoot '..\..\cora-cowork-build-worktrees'),
...
function New-BuildCommandFile([string]$WorktreePath, [string]$Version, [string]$Dsn, [string]$LocalCoraCoworkAppBinary, [string]$LocalCoraCoworkAppBundleDir) {
  ...
  $lines = @(
    ...
    '$env:CORA_COWORK_DEBUG_AUTO_UPDATE_CURRENT_VERSION = ''' + $Version + '''',
    '$env:ELECTRON_BUILDER_COMPRESSION_LEVEL = ''1''',
    '$env:CORA_COWORK_BACKEND_LOCAL_BINARY = ''' + ($LocalCoraCoworkAppBinary -replace "'", "''") + '''',
    '$env:CORA_COWORK_BACKEND_LOCAL_BUNDLE_DIR = ''' + ($LocalCoraCoworkAppBundleDir -replace "'", "''") + '''',
...
function Start-BuildProcess([string]$WorktreePath, [string]$Version, [string]$Dsn, [string]$LogDir, [string]$LocalCoraCoworkAppBinary, [string]$LocalCoraCoworkAppBundleDir) {
  $scriptPath = New-BuildCommandFile $WorktreePath $Version $Dsn $LocalCoraCoworkAppBinary $LocalCoraCoworkAppBundleDir
...
function Wait-BuildProcess($Build, [int]$TimeoutSeconds) {
  ...
  $source = Join-Path $Build.worktreePath "out\CoraCowork-$($Build.version)-win-x64.exe"
  ...
  $target = Join-Path $script:OutputDirResolved "CoraCowork-$($Build.version)-win-x64.exe"
  Copy-Item -LiteralPath $source -Destination $target -Force
...
if (Test-Path -LiteralPath (Join-Path $LocalCoraCoworkAppBundleDir 'managed-resources') -PathType Container) {
  ...
  Write-Host "=== using local CoraCore bundle fallback: $LocalCoraCoworkAppBundleDir ==="
} else {
  $LocalCoraCoworkAppBundleDir = ''
  Write-Warning 'Local CoraCore bundle fallback was not found; builds may need to prepare managed resources.'
}
if (Test-Path -LiteralPath $LocalCoraCoworkAppBinary -PathType Leaf) {
  $LocalCoraCoworkAppBinary = (Resolve-Path -LiteralPath $LocalCoraCoworkAppBinary).Path
  Write-Host "=== using local CoraCore fallback: $LocalCoraCoworkAppBinary ==="
} else {
  $LocalCoraCoworkAppBinary = ''
  Write-Warning 'Local CoraCore fallback was not found; builds may need to download CoraCore.'
}
$worktrees = @()
$builds = @()
$results = @()
...
try {
  foreach ($version in $buildVersions) {
    $worktreePath = Join-Path $runRoot "CoraCowork-$version"
  ...
  foreach ($version in $buildVersions) {
    ...
    $build = Start-BuildProcess $worktreePath $version $dsn $runRoot $LocalCoraCoworkAppBinary $LocalCoraCoworkAppBundleDir
    if ($Sequential) {
