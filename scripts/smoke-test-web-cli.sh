...
if [ ! -d "$TEMP_DIR/cora-cowork-web" ]; then
  echo "❌ Missing cora-cowork-web directory"
  exit 1
fi

cd "$TEMP_DIR/cora-cowork-web"
...
if [ ! -x "cora-cowork-web" ]; then
  echo "❌ cora-cowork-web is not executable"
  exit 1
fi
echo "✓ cora-cowork-web is executable"

# 4. Test version command
echo ""
echo "4. Testing version command..."
VERSION=$(./cora-cowork-web version)
if [ -z "$VERSION" ]; then
...
# CoraCore has no --version flag. Read the pinned version from manifest.json
# (which prepareCoracore writes at pack time) and use --help to confirm the
...
HTTP_PORT=25899
DATA_DIR="$(mktemp -d)/cora-cowork-web-data"
...
./cora-cowork-web start --port "$HTTP_PORT" --data-dir "$DATA_DIR" > /tmp/cora-cowork-web.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${HTTP_PORT}/" > /tmp/cora-cowork-web.html 2>/dev/null; then
    break
  fi
  sleep 1
done

if [ ! -s /tmp/cora-cowork-web.html ]; then
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP probe failed — no response body. Server log:"
  cat /tmp/cora-cowork-web.log
  exit 1
fi

# Look for the SPA shell signature — <html + <div id="root" or similar marker
if grep -q '<html' /tmp/cora-cowork-web.html && grep -qE '<(div id="root"|script)' /tmp/cora-cowork-web.html; then
  echo "✓ HTTP root returns SPA index ($(wc -c < /tmp/cora-cowork-web.html) bytes)"
else
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP root response does not look like SPA index:"
  head -20 /tmp/cora-cowork-web.html
  echo "---server log---"
  cat /tmp/cora-cowork-web.log
  exit 1
fi
...
if grep -q 'Backend binary not found' /tmp/cora-cowork-web.log; then
  ...
  for i in $(seq 1 20); do
    PASSWORD=$(grep -oE 'Generated initial admin password: [^ ]+' /tmp/cora-cowork-web.log | head -1 | sed 's/^Generated initial admin password: //')
    if [ -n "$PASSWORD" ]; then
      break
    fi
    sleep 1
