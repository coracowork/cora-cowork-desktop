 ...
 *
 * Downloads the CoraHub index.json and all extension zip packages
 ...
 *   CORA_COWORK_HUB_TAG    - Git tag to fetch from (default: 'dist-latest')
 *   CORA_COWORK_HUB_SKIP   - Set to '1' to skip hub resource preparation
 */
...
const BASE_URLS = [
  `https://raw.githubusercontent.com/coracowork/CoraHub/${process.env.CORA_COWORK_HUB_TAG || DEFAULT_TAG}/`,
  `https://cdn.jsdelivr.net/gh/coracowork/CoraHub@${process.env.CORA_COWORK_HUB_TAG || DEFAULT_TAG}/`,
];
...
async function prepareHubResources() {
  if (process.env.CORA_COWORK_HUB_SKIP === '1') {
    console.log('[hub] Skipping hub resource preparation (CORA_COWORK_HUB_SKIP=1)');
    return { skipped: true };
  }

  const tag = process.env.CORA_COWORK_HUB_TAG || DEFAULT_TAG;
