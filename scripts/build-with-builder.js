...
/**
 * Simplified build script for CoraCowork
...
const DEBUG_AUTO_UPDATE_CURRENT_VERSION_ENV = 'CORA_COWORK_DEBUG_AUTO_UPDATE_CURRENT_VERSION';
...
function patchElectronBuilderNsisInstaller() {
  ...
  const copiedUninstallerExecWithLog = `ExecWait '"$uninstallerFileNameTemp" /S /KEEP_APP_DATA $0 --installer-log="$CoraCoworkSessionLogPath" --installer-session="$CoraCoworkSessionId" _?=$installationDir' $R0`;
  if (patched.includes(copiedUninstallerExec)) {
    patched = patched.replace(copiedUninstallerExec, copiedUninstallerExecWithLog);
  } else if (
    patched.includes(
      `ExecWait '"$uninstallerFileNameTemp" /S /KEEP_APP_DATA $0 --installer-log="$CoraCoworkSessionLogPath" _?=$installationDir' $R0`
    )
  ) {
  ...
  const inPlaceUninstallerExecWithLog = `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$CoraCoworkSessionLogPath" --installer-session="$CoraCoworkSessionId" _?=$installationDir' $R0`;
  if (patched.includes(inPlaceUninstallerExec)) {
    patched = patched.replace(inPlaceUninstallerExec, inPlaceUninstallerExecWithLog);
  } else if (
    patched.includes(
      `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$CoraCoworkSessionLogPath" _?=$installationDir' $R0`
    )
  ) {
    patched = patched.replace(
      `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$CoraCoworkSessionLogPath" _?=$installationDir' $R0`,
...
function writeGeneratedSentryDsnInclude(projectRoot) {
  ...
  fs.writeFileSync(
    generatedInclude,
    `!define CORA_COWORK_SENTRY_DSN "${escapeNsisDefineValue(process.env.SENTRY_DSN || '')}"\n`
  );
}
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const coraRunning = isProcessRunningWindows('CoraCowork.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (coraRunning || electronRunning) {
        console.log('⚠️  Detected running CoraCowork/Electron process. Attempting to close...');
        killWindowsProcesses(['CoraCowork.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running CoraCowork/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'CoraCowork.exe');
    ...
    console.log('⚠️  Windows local build failed after CoraCowork.exe was produced.');
    if (firstError) {
