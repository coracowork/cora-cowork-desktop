 ...
 * parsing the electron-log file for [CoraCowork:ready] / [CoraCowork:init] /
 * [CoraCowork:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [CoraCowork:ready] marks
  ...
  // Parsed from [CoraCowork:init] marks
  initTotalMs: number;
  // Parsed from [CoraCowork:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'CoraCowork-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'CoraCowork', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'CoraCowork', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'CoraCowork', 'logs', `${today}.log`));
  }
...
// Matches: [CoraCowork:ready] <label> +<ms>ms
// Matches: [CoraCowork:init]  <label> +<ms>ms
// Matches: [CoraCowork:process] <label> +<ms>ms
const MARK_REGEX = /\[CoraCowork:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[CoraCowork] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[CoraCowork] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[CoraCowork] Showing main window')) marks.logs.showingMainWindow = true;
  }
