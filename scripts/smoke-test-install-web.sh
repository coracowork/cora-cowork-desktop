...
export INSTALL_DIR="/tmp/cora-cowork-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/cora-cowork-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/cora-cowork-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/cora-cowork-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/cora-cowork-web"
    exit 1
fi
...
VERSION_OUTPUT=$(cora-cowork-web version 2>&1 || echo "")
