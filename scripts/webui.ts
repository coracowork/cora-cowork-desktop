 ...
 *
 * Env vars:
 *   CORA_COWORK_PORT           : static server port (default 33000)
 *   CORA_COWORK_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   CORA_COWORK_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 *   CORA_COWORK_DATA_DIR       : override userData path (default Electron-compatible)
...
/**
 * Resolve the directory where CoraCore persists its SQLite DB.
 *
 * `bun run webui` runs **independently of the Electron desktop app** — it must
 * work on hosts that never installed CoraCowork.app, and its default work dir must
 * NOT collide with Electron's.
 *
 *   --data-dir <path>       CLI override (highest priority)
 *   $CORA_COWORK_DATA_DIR        env override (same effect)
 *   otherwise               ~/.cora-cowork-web         (production)
 *                           ~/.cora-cowork-web-dev     (dev, default)
 *                           ~/.cora-cowork-web-dev-2   (dev + CORA_COWORK_MULTI_INSTANCE=1)
 *
 * Why a dedicated `-web` name, not the same `~/.cora-cowork[-dev]` that Electron
 * uses: on macOS, Electron's getDataPath() (packages/desktop/src/process/utils/
 * utils.ts) creates `~/.cora-cowork-dev` as a **symlink** to
 * `~/Library/Application Support/CoraCowork-Dev/cora-cowork` so CLI tools (claude,
 ...
 * `.cora-cowork-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.cora-cowork-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.CORA_COWORK_DATA_DIR;
  ...
  const suffix =
    process.env.NODE_ENV === 'production' ? '' : process.env.CORA_COWORK_MULTI_INSTANCE === '1' ? '-dev-2' : '-dev';
  const dir = path.join(os.homedir(), `.cora-cowork-web${suffix}`);
...
function resolveStaticDir(): string {
  if (process.env.CORA_COWORK_STATIC_DIR) return process.env.CORA_COWORK_STATIC_DIR;
  const candidate = path.join(repoRoot, 'out', 'renderer');
  if (fs.existsSync(path.join(candidate, 'index.html'))) return candidate;
  throw new Error(`Renderer assets not found at ${candidate}. Run "bun run package" first, or set CORA_COWORK_STATIC_DIR.`);
}
 ...
 *   $CORA_COWORK_NO_BUILD=1        : env-level opt-out
 *   $CORA_COWORK_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.CORA_COWORK_NO_BUILD)) return;
  if (process.env.CORA_COWORK_STATIC_DIR) return;
...
function resolveBackendBinary(): string {
  if (process.env.CORA_COWORK_BACKEND_BIN) return process.env.CORA_COWORK_BACKEND_BIN;

  const bundledBase = process.env.CORA_COWORK_BACKEND_BUNDLED_DIR ?? path.join(repoRoot, 'resources', 'bundled-cora-cowork');
  ...
  throw new Error(
    `Cannot find "${BACKEND_BINARY}". Set CORA_COWORK_BACKEND_BIN, put it on PATH, or place it at ${bundled}.`
  );
}
...
async function main(): Promise<void> {
  ...
  // This keeps `bun run webui` fully self-contained on hosts without CoraCowork.app.
  const workDir = resolveBackendDataDir();
  const staticDir = resolveStaticDir();
  const backendBin = resolveBackendBinary();
  const logDir = process.env.CORA_COWORK_LOG_DIR ?? path.join(workDir, 'logs');
