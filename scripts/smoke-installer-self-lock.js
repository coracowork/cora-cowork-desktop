...
function main() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'cora-cowork-self-lock-'));
  const installDir = path.join(root, 'install-dir');
  mkdirSync(installDir, { recursive: true });
  writeFileSync(path.join(installDir, 'existing-file.txt'), 'self-lock smoke\n', 'utf8');

  const nsiPath = path.join(root, 'cora-cowork-self-lock-smoke.nsi');
  const exePath = path.join(root, 'cora-cowork-self-lock-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `cora-cowork-installer-self-lock-${new Date()
      .toISOString()
      .replace(/[-:]/g, '')
      .replace(/\..+$/, '')
      .replace('T', '-')}-log.jsonl`
  );
  const resultPath = path.join(process.env.TEMP || tmpdir(), `cora-cowork-installer-self-lock-${process.pid}-result.txt`);
  const processControlPath = path.join(repoRoot, 'resources', 'windows', 'installer-process-control.nsh');

  const nsi = `
Unicode true
Name "CoraCowork Installer Self Lock Smoke"
OutFile "${nsisQuote(exePath)}"
RequestExecutionLevel user
SilentInstall silent
!define VERSION "self-lock-smoke"
!define CORA_COWORK_TARGET_ARCH "x64"
!define CORA_COWORK_FALLBACK_LOG "cora-cowork-installer-self-lock-fallback.log"
!define CORA_COWORK_APP_EXECUTABLE_FILENAME "CoraCowork.exe"
!define UNINSTALL_FILENAME "Uninstall CoraCowork.exe"
!define PROJECT_DIR "${nsisQuote(repoRoot)}"
!include LogicLib.nsh
!include "${nsisQuote(processControlPath)}"

Var CoraCoworkSessionId
Var CoraCoworkIsUpdated
Var CoraCoworkSessionLogPath
Var ResultFile

Section
  StrCpy $INSTDIR "${nsisQuote(installDir)}"
  StrCpy $CoraCoworkSessionId "selflock"
  StrCpy $CoraCoworkIsUpdated "1"
  StrCpy $CoraCoworkSessionLogPath "${nsisQuote(logPath)}"
  StrCpy $ResultFile "${nsisQuote(resultPath)}"
  InitPluginsDir
  SetOutPath $INSTDIR
  StrCpy $CoraCoworkCurrentOutDir "$INSTDIR"
  !insertmacro CORA_COWORK_QUERY_LOCKERS "$INSTDIR" $CoraCoworkLockerResult
  FileOpen $0 "$ResultFile" w
  FileWrite $0 "$CoraCoworkLockerResult"
  FileWrite $0 "|$CoraCoworkCurrentOutDir|$CoraCoworkSessionLogPath"
  FileClose $0
...
`;
  ...
  try {
    ...
    if (!blocking.some((process) => process.name === 'CoraCowork installer' && Number(process.pid) > 0)) {
      throw new Error(`expected CoraCowork installer blocker, got ${JSON.stringify(blocking)}`);
    }
