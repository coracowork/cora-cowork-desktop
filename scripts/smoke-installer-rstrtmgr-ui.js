...
function main() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'cora-cowork-rm-ui-'));
  const installDir = path.join(root, 'install-dir');
  mkdirSync(installDir, { recursive: true });
  const lockedFile = path.join(installDir, 'locked-by-smoke.txt');
  writeFileSync(lockedFile, 'CoraCowork Restart Manager UI smoke lock\n', 'utf8');

  let locker = null;
  const nsiPath = path.join(root, 'cora-cowork-rstrtmgr-ui-smoke.nsi');
  const exePath = path.join(root, 'cora-cowork-rstrtmgr-ui-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `cora-cowork-installer-smoke-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}.log`
  );
  ...
  const nsi = `
Unicode true
Name "CoraCowork Restart Manager UI Smoke"
OutFile "${nsisQuote(exePath)}"
RequestExecutionLevel user
SilentInstall normal
!define CORA_COWORK_FALLBACK_LOG "cora-cowork-installer-smoke-fallback.log"
...
Section
  ...
  StrCpy $CoraCoworkSessionId "rstrtmgrui"
  StrCpy $CoraCoworkIsUpdated "1"
  InitPluginsDir
  BringToFront

  cora-cowork_query_lockers:
    !insertmacro CORA_COWORK_QUERY_LOCKERS "${nsisQuote(lockedFile)}" $CoraCoworkLockerResult
