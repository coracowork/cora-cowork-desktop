#!/usr/bin/env bash
# ============================================================================
# CoraCowork WebUI — One-Click Installation Script
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/coracowork/cora-cowork-desktop/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/cora-cowork-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/cora-cowork-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/coracowork/cora-cowork-desktop/releases/download}"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║     CoraCowork WebUI Installer (No Electron)     ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/cora-cowork-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="cora-cowork-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/coracowork/cora-cowork-desktop/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is cora-cowork-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/cora-cowork-web" ]]; then
        mv "${extract_temp}/cora-cowork-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing cora-cowork-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/cora-cowork-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/cora-cowork-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/cora-cowork-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/cora-cowork-web"
    local target_path="${INSTALL_DIR}/cora-cowork-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by cora-cowork-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 CoraCowork WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/cora-cowork-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start CoraCowork WebUI"
        echo "    cora-cowork-web start"
        echo ""
        echo "    # Check version"
        echo "    cora-cowork-web version"
    else
        echo "    # Start CoraCowork WebUI (using full path)"
        echo "    ${INSTALL_DIR}/cora-cowork-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/cora-cowork-web ~/.local/bin/cora-cowork-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/coracowork/cora-cowork-desktop"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/coracowork/cora-cowork-desktop/issues"
    echo ""
    ...
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo ""
        echo "    # Remove symlink"
        echo "    rm ${BIN_DIR}/cora-cowork-web"
    fi
