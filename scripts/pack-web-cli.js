...
const tarballName = `cora-cowork-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'cora-cowork-web');
...
const executableName = platform === 'win32' ? 'cora-cowork-web.exe' : 'cora-cowork-web';
...
// `cora-cowork-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} cora-cowork-web`, {
