...
const INSTALLER_ERROR_SCENARIOS = [
  {
    ...
    defineName: 'CORA_COWORK_E_UNINSTALLER_COPY_OR_REBUILD_FAILED',
    code: 'E1001',
    message: 'CoraCowork could not repair the installed uninstaller.',
    action: 'Close CoraCowork, restart Windows if needed, then run this installer again.',
    diagnostics:
  ...
  },
  {
    ...
    defineName: 'CORA_COWORK_E_OLD_UNINSTALL_FAILED',
    code: 'E1002',
    message: 'The previous CoraCowork uninstaller returned an error.',
    action:
  ...
  },
  {
    ...
    defineName: 'CORA_COWORK_E_INSTALL_DIR_REMOVE_OR_LOCKED',
    code: 'E1003',
    message: 'CoraCowork could not remove or replace the previous installation directory.',
    action: 'Close CoraCowork and any program using the install folder, then run this installer again.',
    diagnostics: 'scenario=install-dir-remove-or-locked phase=atomic-failed failedPath=install-dir',
  },
  {
    id: 'extract-failed',
    defineName: 'CORA_COWORK_E_EXTRACT_FAILED',
    code: 'E1010',
    message: 'CoraCowork could not extract the application files correctly.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=extract-failed phase=extract method=zip missing=CoraCowork.exe',
  },
  {
    id: 'disk-insufficient',
    defineName: 'CORA_COWORK_E_DISK_INSUFFICIENT',
    code: 'E1020',
    message: 'CoraCowork cannot continue because the target disk does not have enough free space.',
  ...
  {
    id: 'bundled-cora-cowork-incomplete',
    defineName: 'CORA_COWORK_E_BUNDLED_CoraCore_INCOMPLETE',
    code: 'E1030',
    message: 'CoraCowork installed, but the bundled CoraCore resources are incomplete.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=bundled-cora-cowork-incomplete phase=verify-bundled-cora-cowork runtime=win32-x64 result=1',
  },
  {
    id: 'core-app-files-incomplete',
    defineName: 'CORA_COWORK_E_CORE_APP_FILES_INCOMPLETE',
    code: 'E1031',
    message: 'CoraCowork installation is incomplete because a required application file is missing.',
    action: 'Reinstall CoraCowork or download a newer installer.',
    diagnostics: 'scenario=core-app-files-incomplete phase=verify-required-file missing=resources/app.asar',
  },
  {
    id: 'arch-mismatch',
    defineName: 'CORA_COWORK_E_ARCH_MISMATCH',
    code: 'E1040',
    message: 'Installation package architecture mismatch.',
    action: 'Download the CoraCowork installer that matches this Windows architecture, then run it again.',
    diagnostics: 'scenario=arch-mismatch phase=arch-check target=x64 actual=arm64',
  },
  {
    id: 'active-installer-conflict',
    defineName: 'CORA_COWORK_E_ACTIVE_INSTALLER_CONFLICT',
    code: 'E1050',
    message: 'Another CoraCowork installer appears to still be active.',
  ...
  },
  {
    ...
    defineName: 'CORA_COWORK_E_REGISTRY_STATE_INVALID',
    code: 'E1060',
    message: 'CoraCowork found an invalid previous-install registry state.',
    action: 'Uninstall the old CoraCowork from Windows Settings, then run this installer again.',
  ...
  },
  {
    ...
    defineName: 'CORA_COWORK_E_ACTIVE_MARKER_WRITE_FAILED',
    code: 'E1070',
    message: 'CoraCowork could not write the active-installer marker.',
  ...
  },
  {
    ...
    defineName: 'CORA_COWORK_E_INVALID_INSTALL_PATH',
    code: 'E1090',
...
function copyHarnessProject(projectRoot) {
  ...
  writeFileSync(path.join(supportDir, '_sentry-dsn.generated.nsh'), '!define CORA_COWORK_SENTRY_DSN ""\n', 'utf8');
}
...
function readInstallerErrorDefinitions() {
  ...
  const definitions = Array.from(source.matchAll(/!define\s+(CORA_COWORK_E_[A-Z0-9_]+)\s+"(E\d{4})"/g), (match) => ({
    defineName: match[1],
    code: match[2],
  }));
  if (definitions.length === 0) {
    throw new Error('No CORA_COWORK_E_* installer error codes found.');
  }
  return definitions;
}
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*CoraCowork installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "CoraCowork installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "CoraCowork Failure MessageBox Smoke"
...
Section
  StrCpy $INSTDIR "$TEMP\\CoraCowork-messagebox-smoke"
  StrCpy $CoraCoworkSessionId "smokembox-${nsisQuote(scenario.code)}"
  StrCpy $CoraCoworkIsUpdated "1"
  StrCpy $CoraCoworkSessionLogPath "${nsisQuote(logPath)}"
  BringToFront
...
function runHarness({ autoDecline, compileOnly, makensis, scenario }) {
  const { code } = scenario;
  const root = mkdtempSync(path.join(tmpdir(), `cora-cowork-failure-messagebox-${code}-`));
  const projectRoot = path.join(root, 'project');
  const nsiPath = path.join(root, 'cora-cowork-failure-messagebox-smoke.nsi');
  const exePath = path.join(root, 'cora-cowork-failure-messagebox-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `cora-cowork-installer-messagebox-smoke-${code}-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}-log.jsonl`
  );
  const automationPath = path.join(root, 'auto-decline.ps1');
  const reportStatusPath = path.join(process.env.TEMP || tmpdir(), 'cora-cowork-installer-report.json');
  ...
  try {
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (typeof status.copyText !== 'string' || !status.copyText.includes(`CoraCowork installer failure ${code}`)) {
