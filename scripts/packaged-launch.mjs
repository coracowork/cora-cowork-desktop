...
function resolvePackagedApp(projectRoot) {
  ...
  if (process.platform === 'win32') {
    ...
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      ...
      for (const name of ['cora-cowork', 'CoraCowork']) {
...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('CoraCowork.exe');
    await killProcessByName('CoraCowork');
  ...
  const env = {
    ...process.env,
    CORA_COWORK_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] CORA_COWORK_EXTENSIONS_PATH: ${env.CORA_COWORK_EXTENSIONS_PATH}`);
